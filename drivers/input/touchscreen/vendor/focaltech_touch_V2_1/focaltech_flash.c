...
*****************************************************************************/
#ifdef BOOT_FW_UPGRADE_BIN
const struct firmware *fts_fw = NULL;
#else
...
int fts_fwupg_enter_into_boot(struct i2c_client *client)
{
	...
	if (upg->func->pramboot_supported) {
		...
		/* bootloader */
		/* fix FW upgrade failed can't upgrade again */
		#if 0
...
int fts_read_file(char *file_name, u8 **file_buf)
{
	...
	if (IS_ERR(filp)) {
		FTS_ERROR("open %s file fail, try open /etc/firmware/", file_path);
		snprintf(file_path, FILE_NAME_LENGTH, "%s%s", "/etc/firmware/", file_name);
		filp = filp_open(file_path, O_RDONLY, 0);
		if (IS_ERR(filp)) {
...
static int fts_fwupg_get_fw_file(struct fts_ts_data *ts_data)
{
	...
	snprintf(fts_firmware_name, sizeof(fts_firmware_name),
					"fts_firmware_%02x.bin", vendor_id);
	FTS_INFO("fts_firmware_name is %s\n", fts_firmware_name);

	ret = request_firmware(&fts_fw, fts_firmware_name, &ts_data->client->dev);
	if (ret != 0) {
		FTS_ERROR("%s: Firmware image %s not available\n",
				__func__, fts_firmware_name);
		return -ENODATA;
	}
	if (upg) {
		upg->fw = (u8 *)fts_fw->data;
		upg->fw_length = fts_fw->size;
		upg->lic = (u8 *)fts_fw->data;
		upg->lic_length = fts_fw->size;
...
static void fts_fwupg_work(struct work_struct *work)
{
	...
	/* request_firmware takes a lot time, so mv disable-irq postion  */
	fts_irq_disable();
