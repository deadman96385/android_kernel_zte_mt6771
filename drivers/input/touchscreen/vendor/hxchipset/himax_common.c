...
/*{Double Tap, Up, Down, Left, Right, C, Z, M,
	...
	0x08, 0x09, 0x0A, 0x0B, 0x0C, 0x0D, 0x2F,
	};
...
unsigned char finger_down[HIMAX_MAX_TOUCH_NUM] = {0};
...
extern int himax_get_fw_by_lcminfo(void);
#else
extern int himax_get_vendor_id_by_lcdid(struct himax_ts_data *ts, struct himax_i2c_platform_data *pdata);
#endif
extern void himax_free_lcdid_gpio(struct himax_i2c_platform_data *pdata);
extern void himax_tpd_register_fw_class(void);
extern int himax_platform_pinctrl_init(struct himax_i2c_platform_data *pdata);
...
#endif
/*	input_set_abs_params(ts->input_dev, ABS_MT_AMPLITUDE, 0, ((ts->pdata->abs_pressure_max << 16)
|		ts->pdata->abs_width_max), 0, 0);
	input_set_abs_params(ts->input_dev, ABS_MT_POSITION, 0, (BIT(31) | (ts->pdata->abs_x_max << 16)
		| ts->pdata->abs_y_max), 0, 0);*/
...
static int himax_auto_update_check(void)
{
	...
	if (g_core_fp.fp_fw_ver_bin() == 0) {
		I("ic_data->vendor_fw_ver is 0x%x, ic_data->vendor_config_ver is 0x%x!\n",
...
int himax_report_data_init(void)
{
	...
	if (hx_touch_data->raw_cnt_rmd != 0x00) {
		hx_touch_data->rawdata_size = g_core_fp.fp_cal_data_len(hx_touch_data->raw_cnt_rmd,
	...
	if ((ic_data->HX_TX_NUM * ic_data->HX_RX_NUM + ic_data->HX_TX_NUM +
		...
		hx_touch_data->rawdata_frame_size = (ic_data->HX_TX_NUM * ic_data->HX_RX_NUM +
			ic_data->HX_TX_NUM + ic_data->HX_RX_NUM) / hx_touch_data->rawdata_size;
	} else {
		hx_touch_data->rawdata_frame_size = (ic_data->HX_TX_NUM * ic_data->HX_RX_NUM +
			ic_data->HX_TX_NUM + ic_data->HX_RX_NUM) / hx_touch_data->rawdata_size + 1;
	}
	...
	I("%s: ic_data->HX_MAX_PT:%d, hx_raw_cnt_max:%d, hx_raw_cnt_rmd:%d\n",
		__func__, ic_data->HX_MAX_PT, hx_touch_data->raw_cnt_max, hx_touch_data->raw_cnt_rmd);
	I("%s: g_hx_rawdata_size:%d, hx_touch_data->touch_info_size:%d\n",
		__func__, hx_touch_data->rawdata_size, hx_touch_data->touch_info_size);
	...
	hx_touch_data->hx_rawdata_buf = kzalloc(sizeof(uint8_t) * (hx_touch_data->touch_all_size -
...
int himax_parse_report_points(struct himax_ts_data *ts, int ts_path, int ts_status)
{
	...
	if (g_ts_dbg != 0)
		I("%s:finger_num = 0x%2X, finger_on = %d\n", __func__, g_target_report_data->finger_num,
	...
	for (loop_i = 0; loop_i < ts->nFinger_support; loop_i++) {
		...
		if (x >= 0 && x <= ts->pdata->abs_x_max && y >= 0 && y <= ts->pdata->abs_y_max) {
			...
			/* I("%s: g_target_report_data->x[loop_i]=%d, g_target_report_data->y[loop_i]=%d,
				g_target_report_data->w[loop_i]=%d", */
			/*	__func__, g_target_report_data->x[loop_i], g_target_report_data->y[loop_i],
	...
	if (g_ts_dbg != 0) {
		for (loop_i = 0; loop_i < 10; loop_i++) {
			D("DBG X=%d  Y=%d ID=%d\n", g_target_report_data->x[loop_i],
				g_target_report_data->y[loop_i], g_target_report_data->finger_id[loop_i]);
		}
...
#ifdef HX_SMART_AREA
static int himax_smart_area(int w)
{
	if (w > HX_AREA_MIN && w < HX_AREA_0) {
		w = 12;
	} else if (w >= HX_AREA_0 && w < HX_AREA_1) {
		w = 19;
	} else if (w >= HX_AREA_1 && w < HX_AREA_2) {
		w = 21;
	} else if (w >= HX_AREA_2 && w < HX_AREA_3) {
		w = 23;
	} else if (w >= HX_AREA_3 && w < HX_AREA_4) {
		w = 26;
	} else if (w >= HX_AREA_4) {
		w = 28;
	}
...
static void himax_finger_report(struct himax_ts_data *ts)
{
	...
	for (i = 0; i < ts->nFinger_support; i++) {
		if (g_target_report_data->x[i] >= 0 && g_target_report_data->x[i] <= ts->pdata->abs_x_max &&
			...
			valid = false;
			if ((finger_down[i] == 1) && (ts->nFinger_support <= HIMAX_MAX_TOUCH_NUM)) {
				finger_down[i] = 0;
				I("touch up id: %d\n", i);
			}
		}
		...
		if (valid) {
			if (g_ts_dbg != 0)
				I("report_data->x[i]=%d, report_data->y[i]=%d, report_data->w[i]=%d\n",
				g_target_report_data->x[i], g_target_report_data->y[i], g_target_report_data->w[i]);

			if (!finger_down[i] && (ts->nFinger_support <= HIMAX_MAX_TOUCH_NUM)) {
				finger_down[i] = 1;
				I("touch down id: %d, coord [%d:%d]\n",
					i, g_target_report_data->x[i], g_target_report_data->y[i]);
			}

#ifdef HX_SMART_AREA
		g_target_report_data->w[i] = himax_smart_area(g_target_report_data->w[i]);
#endif
...
static void himax_finger_leave(struct himax_ts_data *ts)
{
	...
	for (loop_i = 0; loop_i < ts->nFinger_support; loop_i++) {
		if (((ts->old_finger >> loop_i) & 1) == 1) {
			input_mt_slot(ts->input_dev, loop_i);
			input_mt_report_slot_state(ts->input_dev, MT_TOOL_FINGER, 0);
			if ((finger_down[loop_i] == 1) && (ts->nFinger_support <= HIMAX_MAX_TOUCH_NUM)) {
				finger_down[loop_i] = 0;
				I("touch up id: %d\n", loop_i);
			}
		}
	}
...
int himax_chip_common_init(void)
{
	...
	ret = himax_platform_pinctrl_init(pdata);
	if (ret < 0) {
		E("Failed to init pinctrl\n ");
	}
#ifdef HX_RST_PIN_FUNC
...
#ifdef HIMAX_GET_FW_BY_LCM
	himax_get_fw_by_lcminfo();
#else
	himax_get_vendor_id_by_lcdid(ts, pdata);
#endif
