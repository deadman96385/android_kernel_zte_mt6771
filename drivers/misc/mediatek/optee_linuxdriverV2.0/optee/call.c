...
extern struct wake_lock optee_os_wk_lock;
...
/*
liuliang no need in kernel 3.18
by chenlu begin
*/
/**
					  ...
					  u32 session_id)
{
    ...
    /*pr_info("lllllllllllllllllllllllll find_session sessionid %d\n", session_id); */
	list_for_each_entry(sess, &ctxdata->sess_list, list_node)
		if (sess->session_id == session_id) {
		/*pr_info("lllllllllllllllllllllllll find_session sess->sessionid %d\n", sess->session_id);*/
			return sess;
		}
	return NULL;
}
		      ...
		      struct tee_param *param)
{
	...
	mutex_unlock(&ctxdata->mutex);
	if (!sess) {
		pr_info("lllllllllllllllllllllllll optee_invoke_func ret=EINVAL\n");
		return -EINVAL;
	}

	shm = get_msg_arg(ctx, arg->num_params, &msg_arg, &msg_parg);
	if (IS_ERR(shm)) {
		pr_info("lllllllllllllllllllllllll optee_invoke_func IS_ERR\n");
		return PTR_ERR(shm);
	}
	...
	if (rc) {
		pr_info("lllllllllllllllllllllllll optee_invoke_func optee_to_msg_param fail\n");
		goto out;
	}

	ret = optee_do_call_with_arg(ctx, msg_parg);

	if (ret) {
		pr_info("chenlu [gf_ca_entry] 1 ret=%x\n", ret);
		msg_arg->ret = TEEC_ERROR_COMMUNICATION;
		msg_arg->ret_origin = TEEC_ORIGIN_COMMS;
	}

	ret2 = optee_from_msg_param(param, arg->num_params, msg_param);

	if (ret2) {
		pr_info("chenlu [gf_ca_entry] 2 ret2=%d\n", ret2);
		msg_arg->ret = TEEC_ERROR_COMMUNICATION;
		msg_arg->ret_origin = TEEC_ORIGIN_COMMS;
	}
...
out:
	tee_shm_free(shm);
    /*pr_info("lllllllllllllllllllllllll optee_invoke_func end ret=%d\n", rc);*/
	return rc;
}
