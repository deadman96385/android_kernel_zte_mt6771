...
#include "sp2509vp671f50096_mipi_Sensor.h"

/****************************Modify Following Strings for Debug****************************/
#define PFX "SP2509V_camera_sensor"
#define LOG_1 LOG_INF("SP2509V,MIPI 1LANE\n")
/* #define LOG_2 LOG_INF("preview 1280*960@30fps,420Mbps/lane; video 1280*960@30fps,
	420Mbps/lane; capture 5M@15fps,420Mbps/lane\n") */
/****************************   Modify end    *******************************************/
...
static imgsensor_info_struct imgsensor_info = {
	...
	/* sensor gain delay frame for AE cycle,2
		frame with ispGain_delay-sensor_gain_delay=2-0=2 */
	...
	.sensor_output_dataformat = SENSOR_OUTPUT_FORMAT_RAW_R,/* sensor output mirror filp */
...
static imgsensor_struct imgsensor = {
	...
	.test_pattern = KAL_FALSE,        /* test pattern mode or not. KAL_FALSE for in
	test pattern mode, KAL_TRUE for normal output */
...
static SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[6] = {
	...
	{  1600, 1200,  0, 0, 1600, 1200, 1600, 1200, 0000, 0000, 1600, 1200,  0, 0, 1600, 1200}, /* high video */
...
#define DEBUG_SENSOR
#ifdef DEBUG_SENSOR
	#define sp2509vMIPI_OP_CODE_INI		0x00		/* Initial value. */
	#define sp2509vMIPI_OP_CODE_REG		0x01		/* Register */
	#define sp2509vMIPI_OP_CODE_DLY		0x02		/* Delay */
	#define sp2509vMIPI_OP_CODE_END		0x03		/* End of initial setting. */
	typedef struct {
		u16 init_reg;
		u16 init_val;	/* Save the register value and delay tick */
		u8 op_code;		/* 0 - Initial value, 1 - Register, 2 - Delay, 3 - End of setting. */
	} sp2509vMIPI_initial_set_struct;

	sp2509vMIPI_initial_set_struct sp2509vMIPI_Init_Reg[1000];
...
u8 sp2509vMIPI_Initialize_from_T_Flash(void)
{
	...
	fp = filp_open("/mnt/sdcard/sp2509v_sd", O_RDONLY, 0);
	if (IS_ERR(fp)) {
		LOG_INF("create file error\n");
	}
	fs = get_fs();
	set_fs(KERNEL_DS);
	...
	while (curr_ptr < (data_buff + file_size)) {
		...
		if (strcmp((const char *)func_ind, "REG") == 0) {
			...
			sp2509vMIPI_Init_Reg[i].op_code = sp2509vMIPI_OP_CODE_REG;

			sp2509vMIPI_Init_Reg[i].init_reg = strtol((const char *)curr_ptr, 16);
			curr_ptr += 5;	/* Skip "00, 0x" */

			sp2509vMIPI_Init_Reg[i].init_val = strtol((const char *)curr_ptr, 16);
			curr_ptr += 4;	/* Skip "00);" */

		} else {
			/* Need add delay for this setting. */
			curr_ptr += 4;
			sp2509vMIPI_Init_Reg[i].op_code = sp2509vMIPI_OP_CODE_DLY;

			sp2509vMIPI_Init_Reg[i].init_reg = 0xFF;
			/* Get the delay ticks, the delay should less then 50 */
			sp2509vMIPI_Init_Reg[i].init_val = strtol((const char *)curr_ptr,  10);
		}
		i++;
	...
	sp2509vMIPI_Init_Reg[i].op_code = sp2509vMIPI_OP_CODE_END;
	sp2509vMIPI_Init_Reg[i].init_reg = 0xFF;
	sp2509vMIPI_Init_Reg[i].init_val = 0xFF;
	i++;
	/* for (j=0; j<i; j++)
		printk(" %x  ==  %x\n",sp2509vMIPI_Init_Reg[j].init_reg, sp2509vMIPI_Init_Reg[j].init_val); */

	/* Start apply the initial setting to sensor. */
	for (j = 0; j < i; j++) {
		if (sp2509vMIPI_Init_Reg[j].op_code == sp2509vMIPI_OP_CODE_END)	/* End of the setting. */ {
			break;
		} else if (sp2509vMIPI_Init_Reg[j].op_code == sp2509vMIPI_OP_CODE_DLY) {
			msleep(sp2509vMIPI_Init_Reg[j].init_val);		/* Delay */
		} else if (sp2509vMIPI_Init_Reg[j].op_code == sp2509vMIPI_OP_CODE_REG) {

			write_cmos_sensor(sp2509vMIPI_Init_Reg[j].init_reg, sp2509vMIPI_Init_Reg[j].init_val);
			LOG_INF("%x = %x\n", sp2509vMIPI_Init_Reg[j].init_reg, sp2509vMIPI_Init_Reg[j].init_val);
		} else {
			LOG_INF("REG ERROR!\n");
		}
...
static kal_uint16 set_gain(kal_uint16 gain)
{
	...
	if (gain >= BASEGAIN && gain <= 15*BASEGAIN) {
		...
		if (iReg <= 0x10) {
			...
			write_cmos_sensor(0x01, 0x01);
			LOG_INF("SP2509V_MIPI_SetGain = 16");
		} else if (iReg >= 0xa0) /* gpw */ {
			write_cmos_sensor(0xfd, 0x01);
			write_cmos_sensor(0x24, 0xa0);
			write_cmos_sensor(0x01, 0x01);
			LOG_INF("SP2509V_MIPI_SetGain = 160");
		} else {
			write_cmos_sensor(0xfd, 0x01);
			write_cmos_sensor(0x24, (kal_uint8)iReg);
			write_cmos_sensor(0x01, 0x01);
			LOG_INF("SP2509V_MIPI_SetGain = %d", iReg);
		}
	} else
...
if (fromsd == 1) /*是否从SD读取 */ {
	...
	sp2509vMIPI_Initialize_from_T_Flash(); /* 从SD卡读取的主要函数 */
} else
#endif
	...
	write_cmos_sensor(0x0a, 0x19);
	write_cmos_sensor(0x05, 0x00);
	write_cmos_sensor(0x06, 0x2c);
	...
	write_cmos_sensor(0xfd, 0x01);
	write_cmos_sensor(0x9d, 0x96);/*ea*/
	...
	write_cmos_sensor(0xfd, 0x01);
	write_cmos_sensor(0xf0, 0xfd);/* tmp begin */
	...
	write_cmos_sensor(0xf7, 0x00);/* tmp end */
	...
	write_cmos_sensor(0xfd, 0x01);
	write_cmos_sensor(0x3f, 0x03);/* mirror filp*/
...
static kal_uint32 open(void)
{
	...
	/* #ifdef DEBUG_SENSOR  */
		...
		if (IS_ERR(fp)) {
			fromsd = 0;
			LOG_INF("gpww open file error\n");
		}

	    else {
			fromsd = 1;
			/* sp2509vMIPI_Initialize_from_T_Flash(); */
			LOG_INF("gpww read ok!\n");
...
static kal_uint32 streaming_control(kal_bool enable)
{
	...
	if (enable) {
		write_cmos_sensor(0xfd, 0x01);
		write_cmos_sensor(0xac, 0x01);
		LOG_INF("sp2509v hts %d, vts %d", (read_cmos_sensor(0x8c)<<8)|(read_cmos_sensor(0x8d)),
...
UINT32 SP2509VP671F50096_MIPI_RAW_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
	/* To Do : Check Sensor status here */
	if (pfFunc != NULL)
		*pfFunc = &sensor_func;
	return ERROR_NONE;
}    /*    SP2509V_MIPISensorInit    */
