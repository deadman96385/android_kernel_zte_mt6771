...
void mt_mt65xx_led_set(struct led_classdev *led_cdev, enum led_brightness level)
{
	...
	static int is_aal_ess_disabled = -1;
#endif
#ifdef CONFIG_ZTE_LCD_BACKLIGHT_LEVEL_CURVE
...
#ifdef AAL_SUPPORT_PARTIAL_UPDATE
	if (level != 0) {
		if (level <= 20 && (is_aal_ess_disabled == 0 || is_aal_ess_disabled == -1)) {
			pr_info("%s: lcm backlight level = %d disp_aal_set_ess_en 0\n", __func__, level);
			disp_aal_set_ess_en(0);
			is_aal_ess_disabled = 1;
		} else if (level > 20 && (is_aal_ess_disabled == 1 || is_aal_ess_disabled == -1)) {
			pr_info("%s: lcm backlight level = %d disp_aal_set_ess_en 1\n", __func__, level);
			disp_aal_set_ess_en(1);
			is_aal_ess_disabled = 0;
		}
	} else {
		is_aal_ess_disabled = -1;
	}
