...
#define cts_err(fmt, ...)    pr_err("<E>CTS-" LOG_TAG " " fmt"\n", ##__VA_ARGS__)
#define cts_warn(fmt, ...)   pr_info("<W>CTS-" LOG_TAG " " fmt"\n", ##__VA_ARGS__)
#define cts_info(fmt, ...)  pr_info("<I>CTS-" LOG_TAG " " fmt"\n", ##__VA_ARGS__)
#define cts_dbg(fmt, ...)   \
	do { \
		if (cts_show_debug_log)  \
		pr_info("<D>CTS-" LOG_TAG " "fmt"\n", ##__VA_ARGS__);   \
	} while (0)
...
extern u8 *cts_plat_get_i2c_xfer_buf(struct cts_platform_data *pdata, size_t xfer_size);
...
extern int cts_init_platform_data(struct cts_platform_data *pdata, struct i2c_client *i2c_client);
...
extern int cts_plat_process_touch_msg(struct cts_platform_data *pdata, struct cts_device_touch_msg *msgs, int num);
...
static inline int cts_plat_process_vkey(struct cts_platform_data *pdata, u8 vkey_state)
{
	return 0;
}
...
extern int cts_plat_process_gesture_info(struct cts_platform_data *pdata, struct cts_device_gesture_info *gesture_info);
