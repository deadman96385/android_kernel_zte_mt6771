...
#define WAKELOCK_HOLD_TIME 800 /* in ms */
...
static void gf_spi_clk_enable(struct gf_device *gf_dev, u8 bonoff)
{
	...
	if (bonoff && (count == 0)) {
		gf_debug(DEBUG_LOG, "%s, start to enable spi clk && count = %d.\n", __func__, count);
		mt_spi_enable_master_clk(gf_dev->spi);    /* FOR MT6797 */
		count = 1;
	} else if ((count > 0) && (bonoff == 0)) {
		gf_debug(DEBUG_LOG, "%s, start to disable spi clk&& count = %d.\n", __func__, count);
		mt_spi_disable_master_clk(gf_dev->spi);    /* FOR MT6797 */
		count = 0;
	}
#endif
#endif
...
static void gf_netlink_recv(struct sk_buff *__skb)
{
	...
	if (skb->len >= NLMSG_SPACE(0)) {
		...
		memcpy(str, NLMSG_DATA(nlh), GF_MIN(data_len, (uint32_t)sizeof(str)));
		pid = nlh->nlmsg_pid;
...
static long gf_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	*/
	...
	case GF_IOC_INPUT_KEY_EVENT:
		...
		if ((gf_key.key == GF_KEY_POWER || gf_key.key == GF_KEY_CAMERA) && (gf_key.value == 1)) {
...
#ifdef SUPPORT_TEE_GF_IOC_REMOVE
...
static int gf_check_gf3658_chip_id(struct gf_device *gf_dev)
{
	u32 time_out = 0;
	u8 tmp_buf[8] = {0};
	...
	do {
		...
		if ((tmp_buf[2] == 0x25) && (tmp_buf[1] == 0x03) && (tmp_buf[0] == 0xa8)) {
			gf_debug(INFO_LOG, "%s, gf3658 chip id check pass, time_out=%d\n", __func__, time_out);
			return 0;
		}
	} while (time_out < 3);

	gf_debug(INFO_LOG, "%s, chip id read failed, time_out=%d\n", __func__, time_out);
...
int gf_spi_read_bytes_ree(struct gf_device *gf_dev, u16 addr, u32 data_len, u8 *rx_buf)
{
	...
	if (xfer == NULL) {
		gf_debug(ERR_LOG, "%s, failed alloc for SPI transfer\n", __func__);
		return -ENOMEM;
	}
...
static int gf_probe(struct spi_device *spi)
{
	...
	gf_dev->spi_buffer = kzalloc(bufsiz + 256, GFP_KERNEL);
	if (gf_dev->spi_buffer == NULL) {
		status = -ENOMEM;
		goto err_buf;
	}
...
#ifdef SUPPORT_REE_SPI
#ifdef SUPPORT_REE_MILAN_A
	gf_spi_read_bytes_ree_new(gf_dev, 0x0000, 2, tmp_buf);
