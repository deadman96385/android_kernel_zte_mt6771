...
{
    unsigned char (*InvalidNode)[RX_NUM_MAX];// Invalid node, the node is not tested
    unsigned char (*InvalidNode_SC)[RX_NUM_MAX];//Invalid node, the SCAP node is not tested
    ...
    int (*SCapRawDataTest_ON_Max)[RX_NUM_MAX];//Starting from this is the Self Capacitance test item, the Mutual Capacitance of each test item must be put on the top.
