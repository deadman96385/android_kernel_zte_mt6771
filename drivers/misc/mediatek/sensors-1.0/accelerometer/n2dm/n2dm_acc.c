...
static int n2dm_read_chip_name(struct n2dm_data *obj, u8 *buf, int bufsize)
{
	...
	snprintf(buf, bufsize, "N2DM Chip");
	return 0;
}
/*----------------------------------------------------------------------------*/
static int n2dm_acc_read_data(struct n2dm_acc *acc_obj, u8 *buf, int bufsize)
{
	...
	snprintf(buf, bufsize, "%04x %04x %04x", acc[N2DM_AXIS_X], acc[N2DM_AXIS_Y], acc[N2DM_AXIS_Z]);
...
/*----------------------------------------------------------------------------*/
static int n2dm_acc_read_rawdata_string(struct n2dm_acc *acc_obj, u8 *buf, int bufSize)
{
	struct n2dm_data *obj = container_of(acc_obj, struct n2dm_data, n2dm_acc_data);
	struct i2c_client *client = obj->client;
	int res = 0;
	...
	snprintf(buf, bufSize, "%04x %04x %04x", acc_obj->data[N2DM_AXIS_X],
...
static ssize_t n2dm_attr_acc_show_rawdata_value(struct device_driver *ddri, char *buf)
{
	...
	n2dm_acc_read_rawdata_string(acc_obj, strbuf, N2DM_BUFSIZE);
	return snprintf(buf, PAGE_SIZE, "%s\n", strbuf);
}
/*----------------------------------------------------------------------------*/
