...
static void ilitek_tddi_wq_bat_check(struct work_struct *work)
{
	...
	if (strnstr(str, "Charging", WQ_BAT_STR_SIZE) != NULL
		|| strnstr(str, "Full", WQ_BAT_STR_SIZE) != NULL
		|| strnstr(str, "Fully charged", WQ_BAT_STR_SIZE) != NULL) {
		if (charge_mode != 1) {
...
int ilitek_tddi_sleep_handler(int mode)
{
	...
	if (atomic_read(&idev->fw_stat) ||
		...
		ipio_info("fw upgrade or mp still running, ignore sleep request\n");
...
void ilitek_tddi_report_handler(void)
{
	...
	case P5_X_FW_GESTURE_MODE:
		...
		break;
	case P5_X_FW_DEMO_DEBUG_INFO_MODE:
		/*only support SPI interface now, so default use size 1024 buffer*/
		rlen = 1024;
		break;
	default:
