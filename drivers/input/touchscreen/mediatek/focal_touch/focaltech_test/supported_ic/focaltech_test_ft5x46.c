...
*******************************************************************************/
#define IC_TEST_VERSION  "Test version: V1.1.0--2016-08-01, (sync version of FT_MultipleTest: V2.7.0.3--2015-07-13)"
...
static unsigned char g_iIcValue = 0;

/*******************************************************************************
* Global variable or extern global variabls/functions
*******************************************************************************/
extern int g_int_tptest_result;
...
extern struct stCfg_FT5X45_BasicThreshold g_stCfg_FT5X46_BasicThreshold;
...
boolean FT5X46_StartTest()
{
	...
	ReadReg(0xB1, &g_iIcValue);//Get IC type
	FTS_TEST_DBG("%s get ic value:%d", __func__, g_iIcValue);
	...
	for(iItemCount = 0; iItemCount < g_TestItemNum; iItemCount++)
	{
		...
		{
			FTS_TEST_DBG("Code_FT5X46_ENTER_FACTORY_MODE");
		...
		///////////////////////////////////////////////////////FT5X46_CHANNEL_NUM_TEST
		/*if(Code_FT5X46_CHANNEL_NUM_TEST == g_stTestItem[ucDevice][iItemCount].ItemCode
		)
		{
		...
		{
			FTS_TEST_DBG("Code_FT5X46_RAWDATA_TEST");
			ReCode = FT5X46_TestItem_RawDataTest(&bTempResult);
			if(ERROR_CODE_OK != ReCode || (!bTempResult))
			{
				bTestResult = false;
				g_stTestItem[ucDevice][iItemCount].TestResult = RESULT_NG;
				g_int_tptest_result = g_int_tptest_result | TEST_BEYOND_MAX_LIMIT | TEST_BEYOND_MIN_LIMIT;
				printk("tpd rawdata failed\n");
			}
			else
				g_stTestItem[ucDevice][iItemCount].TestResult = RESULT_PASS;
		}
		...
		{
			FTS_TEST_DBG("Code_FT5X46_SCAP_CB_TEST \n");
		...
		{
			FTS_TEST_DBG("Code_FT5X46_SCAP_RAWDATA_TEST \n");
		...
		if(Code_FT5X46_UNIFORMITY_TEST == g_stTestItem[ucDevice][iItemCount].ItemCode
			)
		{
			FTS_TEST_DBG("Code_FT5X46_UNIFORMITY_TEST \n");
			ReCode = FT5X46_TestItem_UniformityTest(&bTempResult);
			if(ERROR_CODE_OK != ReCode || (!bTempResult))
			{
				bTestResult = false;
				g_stTestItem[ucDevice][iItemCount].TestResult = RESULT_NG;
				g_int_tptest_result = g_int_tptest_result | TEST_BEYOND_ACCORD_LIMIT;
				printk("tpd uniformity failed\n");
			}
			else
				g_stTestItem[ucDevice][iItemCount].TestResult = RESULT_PASS;
		}
		...
		{
			FTS_TEST_DBG("Code_FT5X46_WEAK_SHORT_CIRCUIT_TEST \n");
		...
		{
			FTS_TEST_DBG("Code_FT5X46_PANELDIFFER_TEST \n");
	...
	printk("tpd 0x%x", g_int_tptest_result);
...
int FT5X46_get_raw_data(int *data, int length, int cmd, int type)
{
	unsigned char ReCode = 0;
	...
	//int RawDataMax;
	//unsigned char ucFre;
	static unsigned char strSwitch = 0;
	static unsigned char OriginValue = 0xff;
	//int index = 0;
	//int iRow, iCol;
	//int iValue = 0;
	//int nRawDataOK = 0;
	...
	if( 1== cmd) {
		...
		//The rawdata after equalization，default 0X16=0
		...
		FTS_TEST_DBG( "\n=========FIR State: ON");
		ReCode = WriteReg(0xFB, 1);//FIR OFF  0：OFF， 1：ON
		if(ReCode != ERROR_CODE_OK)		
		{
	...
	if(3 == cmd) {
		...
		ShowRawData();
		if(length < g_ScreenSetParam.iTxNum * g_ScreenSetParam.iRxNum) {
			FTS_TEST_DBG("error lenth is %d, need:%d\n",  length, g_ScreenSetParam.iTxNum * g_ScreenSetParam.iRxNum);
			goto TEST_ERR;
		}
		...
		for(i = 0; i < g_ScreenSetParam.iTxNum; i++)
		{
			...
			{
				data[i * g_ScreenSetParam.iRxNum + j] = m_RawData[i][j];
			}
		}
		ReCode = g_ScreenSetParam.iTxNum * g_ScreenSetParam.iRxNum;
	}
...
TEST_ERR:
	FTS_TEST_DBG("	FT5X46_get_raw_data failed");
	return -1;

}
int FT5X46_get_differ_data(int *data, int length, int cmd, int type)
{
	//int index = 0;
	//int iRow = 0, iCol = 0;
	//int iValue = 0;
	...
	static unsigned char OriginRawDataType = 0xff;
	static unsigned char OriginFrequecy = 0xff;
	static unsigned char OriginFirState = 0xff;

	FTS_TEST_DBG("==============================FT5X46_get_differ_data -------- \n");
	if( 1== cmd) {
		ReCode = EnterFactory(); 
		SysDelay(20);
		if(ReCode != ERROR_CODE_OK)		
	...
	if(3 == cmd) {
		...
		for(i = 0; i < g_ScreenSetParam.iTxNum; i++)
		{
			for( j= 0; j < g_ScreenSetParam.iRxNum; j++)
			{
				data[i * g_ScreenSetParam.iRxNum + j] = m_RawData[i][j]/10;
			}
		}
		ReCode = g_ScreenSetParam.iTxNum * g_ScreenSetParam.iRxNum;
	}
...
TEST_ERR:
	FTS_TEST_DBG("	FT5X46_get_differ_data failed");
	return -1;
}
int FT5X46_get_node_data(int *data, int length, int cmd, int type)
{
	...
	if(1 == type) {
		iLen = FT5X46_get_raw_data(data, length, cmd, type);
	} else if(2 == type){
		iLen = FT5X46_get_differ_data(data, length, cmd, type);
	}
...
//Data of High Frequec and FIR=1
unsigned char FT5X46_TestItem_UniformityTest(bool * bTestResult)
{
	unsigned char ReCode = ERROR_CODE_COMM_ERROR;
	bool btmpresult = true;
...
//	int shieldNode[TX_NUM_MAX][RX_NUM_MAX] = {0};
	...
	if( g_stCfg_FT5X46_BasicThreshold.Uniformity_CheckTx )
	{
	        FTS_TEST_DBG("\r\n=========Check Tx Linearity \r\n");
		for ( iRow = 0; iRow < g_ScreenSetParam.iTxNum; ++iRow )
		{
		...
		FTS_TEST_DBG("");
			...
			*/
			...
			{
			//	printk("Tx%2d:  		"  , iRow);
				//for(iCol = 0; iCol < g_ScreenSetParam.iRxNum; iCol++)    //modify 20170109 for skip key rx check
	...
	if( g_stCfg_FT5X46_BasicThreshold.Uniformity_CheckRx )
	{
		FTS_TEST_DBG("\r\n=========Check Rx Linearity \r\n");
	...
	if( g_stCfg_FT5X46_BasicThreshold.Uniformity_CheckMinMax )
	{
        	FTS_TEST_DBG("\r\n=========Check Min/Max \r\n") ;
		iMin = 100000;
		iMax = -100000;
		...
		if( iUniform < g_stCfg_FT5X46_BasicThreshold.Uniformity_MinMax_Hole )
		{
			btmpresult = false;
			FTS_TEST_DBG("\r\n MinMax Out Of Range, Set Value: %d", g_stCfg_FT5X46_BasicThreshold.Uniformity_MinMax_Hole  );
		}
...
*/
	...
	///////////////////////Channel and Ground
	///////////////////////Channel and Channel
//	strAdc = "\r\Channel and Ground:\r\n";
//	strTemp = "\r\n\r\Channel and Channel：\r\n";
...
unsigned char FT5X46_TestItem_PanelDifferTest(bool * bTestResult)
{
	...
	*/
	...
	///whether threshold is in range
	////////////////////////////////To Determine  if in Range or not
	iMin =  g_stCfg_FT5X46_BasicThreshold.PanelDifferTest_Min;		//	g_stCfg_FT5X46_BasicThreshold.PanelDifferTest_Min[iRow][iCol];
	iMax = g_stCfg_FT5X46_BasicThreshold.PanelDifferTest_Max;
