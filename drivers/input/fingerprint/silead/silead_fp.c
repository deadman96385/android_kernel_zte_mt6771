...
#define SILEAD_UUID "a8224991-8c4f-42fc-8cbe-623821ecc211"
...
#ifdef NAVI_ORIENTATION_MODIFY
	{ NAV_KEY_UP,         SL_NAV_REAL_DOWN,  }, /* KEY_RESERVED, ignore this key */
	{ NAV_KEY_DOWN,       SL_NAV_REAL_UP,    },
...
static int silfp_probe(struct spi_device *spi)
{
	...
	if (!fp_dev) {
		LOG_MSG_DEBUG(ERR_LOG, "[%s] [%d] kzalloc failed.\n", __func__, __LINE__);
		return -ENOMEM;
	}
	...
	*/
	...
	if (status < 0) {
		...
		LOG_MSG_DEBUG(INFO_LOG, "[%s], major=%d, minor=%d\n", __func__,
			MAJOR(fp_dev->devt), MINOR(fp_dev->devt));
	}
	...
	LOG_MSG_DEBUG(INFO_LOG, "[%s]probe ok.\n", __func__);
...
static int __init silfp_dev_init(void)
{
	...
	LOG_MSG_DEBUG(INFO_LOG, "[%s]SILEAD_FP Driver create spi2, Version: %s.\n", __func__, FP_DEV_VERSION);
	...
	*/
	...
	if (status < 0) {
		LOG_MSG_DEBUG(ERR_LOG, "[%s] register_chrdev  failed status = %d\n", __func__, status);
		return status;
	}
	...
	if (IS_ERR(silfp_class)) {
		LOG_MSG_DEBUG(ERR_LOG, "[%s] class_create silfp_class failed\n", __func__);
	...
	LOG_MSG_DEBUG(INFO_LOG, "[%s] SILEAD_FP_DRIVER dev_init success\n", __func__);
	return status;
}
