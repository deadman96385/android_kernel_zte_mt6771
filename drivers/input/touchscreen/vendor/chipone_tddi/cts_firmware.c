...
static int calc_crc_in_flash(const struct cts_device *cts_dev, u32 flash_addr, size_t size, u32 *crc)
{
	return cts_dev->hwdata->sfctrl->ops->calc_flash_crc(cts_dev, flash_addr, size, crc);
}
...
{
	return (firmware->size > 0x102 && firmware->size <= CTS_FIRMWARE_MULTI_SECTION_FILE_SIZE);
}
...
static bool is_firmware_valid(const struct cts_firmware *firmware)
{
	if (firmware && firmware->data && is_firmware_size_valid(firmware)) {
		if (is_single_section_firmware(firmware) || is_multi_section_firmware_valid(firmware)) {
			return true;
		}
	}
...
static void parse_single_section_firmware(const struct cts_firmware *firmware, struct cts_firmware_sect_info *info)
{
...
static void parse_multi_section_firmware(const struct cts_firmware *firmware, struct cts_firmware_sect_info *info)
{
...
static int parse_firmware(const struct cts_firmware *firmware, struct cts_firmware_sect_info *info)
{
...
const struct cts_firmware *cts_request_driver_builtin_firmware_by_index(u32 index)
{
	...
	if (index < NUM_DRIVER_BUILTIN_FIRMWARE) {
		...
		cts_warn("Request driver builtin by index %u too large >= %zu", index, NUM_DRIVER_BUILTIN_FIRMWARE);
	}
...
static const struct cts_firmware *cts_request_newer_driver_builtin_firmware(u16 hwid, u16 fwid, u16 device_fw_ver)
{
#define MATCH_HWID(firmware, hwid) \
	((hwid) == CTS_HWID_ANY || (firmware)->hwid == (hwid))
#define MATCH_FWID(firmware, fwid) \
	((fwid) == CTS_FWID_ANY || (firmware)->fwid == (fwid))

	const struct cts_firmware *firmware = NULL;
	int i;

	cts_info("Request driver builtin if match hwid: %04x fwid: %04x && ver > %04x", hwid, fwid, device_fw_ver);
...
const struct cts_firmware *cts_request_newer_firmware_from_system(struct cts_device *cts_dev,
			char *filepath, u16 curr_version)
{
	...
	ret = request_firmware(&fw, filepath, &cts_dev->pdata->i2c_client->dev);
	if (ret) {
		cts_err("request firmware failed:%s", filepath);
		goto err_free_firmware;
	}
	firmware->size = fw->size;
	version = *((u16 *)(fw->data + FIRMWARE_VERSION_OFFSET));
	cts_info("request file fw version:0x%04x", version);

	if (version <= curr_version) {
		cts_info("File '%s' size: %zu version: %04x <= %04x", filepath, firmware->size, version, curr_version);
		goto err_free_fw;
	}

	cts_info("File '%s' size: %zu version: %04x", filepath, firmware->size, version);
	...
	memcpy(firmware->data, fw->data, fw->size);
...
const struct cts_firmware *cts_request_newer_firmware_from_fs(struct cts_device *cts_dev,
			const char *filepath, u16 curr_version)
{
	...
	if (kernel_read(file, FIRMWARE_VERSION_OFFSET, buff, 2) < 0) {
...
const struct cts_firmware *cts_request_firmware_from_fs(struct cts_device *cts_dev, const char *filepath)
{
...
int cts_update_firmware_from_file(struct cts_device *cts_dev, const char *filepath, bool to_flash)
{
	const struct cts_firmware *firmware;
	int ret;

	cts_info("Update from file '%s' to %s", filepath, to_flash ? "flash" : "sram");
	...
	if (ret) {
		cts_err("Update to %s from file failed %d", to_flash ? "flash" : "sram", ret);
...
const struct cts_firmware *cts_request_firmware(struct cts_device *cts_dev, u16 hwid, u16 fwid, u16 curr_firmware_ver)
{
	...
	cts_info("Request newer if match hwid: %04x fwid: %04x && ver > %04x", hwid, fwid, curr_firmware_ver);

#ifdef CFG_CTS_DRIVER_BUILTIN_FIRMWARE
	firmware_builtin = cts_request_newer_driver_builtin_firmware(hwid, fwid, curr_firmware_ver);
#endif /* CFG_CTS_DRIVER_BUILTIN_FIRMWARE */

#ifdef CFG_CTS_FIRMWARE_IN_FS
	/* Check firmware in file system when probe*/
	firmware_from_file = cts_request_newer_firmware_from_system(cts_dev, cts_firmware_name,
								firmware_builtin ?
								FIRMWARE_VERSION(firmware_builtin) :
								curr_firmware_ver);
#endif /* CFG_CTS_FIRMWARE_IN_FS */
...
static int cts_program_firmware(const struct cts_device *cts_dev, const struct cts_firmware_sect_info *firmware_info)
{
	int ret;
	...
	ret = cts_program_flash_from_sram(cts_dev, 4, 4, firmware_info->firmware_sect_size - 4);
	if (ret) {
	...
	ret = cts_program_flash(cts_dev,
				...
				firmware_info->firmware_crc_sect, firmware_info->firmware_crc_sect_size);
	if (ret) {
	...
	ret = validate_flash_data(cts_dev,
				  ...
				  firmware_info->firmware_sect_size, NULL, true, firmware_info->firmware_sect_crc);
	if (ret) {
...
static int cts_program_ddiparam(const struct cts_device *cts_dev, const struct cts_firmware_sect_info *firmware_info)
{
	int ret;
	...
	ret = validate_flash_data(cts_dev,
				  ...
				  firmware_info->ddiparam_sect_size, NULL, true, firmware_info->ddiparam_sect_crc);
	if (ret) {
	...
	ret = cts_program_flash(cts_dev, CTS_DDIPARAM_CRC_SECTION_OFFSET,
				firmware_info->ddiparam_crc_sect, firmware_info->ddiparam_crc_sect_size);
	if (ret) {
...
int cts_update_firmware(struct cts_device *cts_dev, const struct cts_firmware *firmware, bool to_flash)
{
	struct cts_firmware_sect_info firmware_info;
	int ret, retries;

	cts_info("Update firmware to %s ver: %04x size: %zu",
		 to_flash ? "flash" : "sram", FIRMWARE_VERSION(firmware), firmware->size);
	...
	if (!to_flash) {
		cts_info("Write firmware section to sram size %zu", firmware_info.firmware_sect_size);
		ret = cts_sram_writesb_check_crc_retry(cts_dev,
						       0, firmware_info.firmware_sect, firmware_info.firmware_sect_size,
						       firmware_info.firmware_sect_crc, 3);
		if (ret) {
	...
	do {
		...
		ret = cts_erase_flash(cts_dev, CTS_FIRMWARE_CRC_SECTION_OFFSET, firmware_info.firmware_crc_sect_size);
		if (ret) {
			cts_err("Erase firmware crc section failed %d retries %d", ret, retries);
			continue;
		}

		ret = cts_erase_flash(cts_dev, 0, firmware_info.firmware_sect_size);
		if (ret) {
			cts_err("Erase firmware section failed %d retries %d", ret, retries);
			continue;
		}

		ret = cts_program_firmware(cts_dev, &firmware_info);
		if (ret) {
			cts_err("Program firmware & crc section failed %d retries %d", ret, retries);
		}
	} while (ret && retries < 3);
	...
	if (ret == 0 && firmware_info.ddiparam_sect_size != 0) {
		...
		do {
			...
			if (ret) {
				cts_err("Erase DDIParam crc secction failed %d, retries %d", ret, retries);
				continue;
			}

			ret = cts_erase_flash(cts_dev, CTS_DDIPARAM_SECTION_OFFSET, firmware_info.ddiparam_sect_size);
			if (ret) {
				cts_err("Erase DDIParam section failed %d, retries %d", ret, retries);
				continue;
			}

			ret = cts_program_ddiparam(cts_dev, &firmware_info);
			if (ret) {
				cts_err("Program DDIParam & crc section failed %d retries %d", ret, retries);
			}
		} while (ret && retries < 3);
	}
...
out:
	...
	if (ret == 0) {
		if (firmware_info.firmware_sect_size <= cts_dev->hwdata->sfctrl->xchg_sram_base) {
