...
#include "sp2609mipi_Sensor.h"

#define PFX "SP2609_camera_sensor"
...
static imgsensor_info_struct imgsensor_info = {
	.sensor_id = SP2609_SENSOR_ID,

	.checksum_value = 0x619b6a99,

	.pre = {
		.pclk = 39000000,            /*record different mode's pclk*/
		.linelength = 1017,            /*record different mode's linelength*/
		.framelength = 1217,            /*record different mode's framelength*/
...
static imgsensor_struct imgsensor = {
	...
	.autoflicker_en = KAL_FALSE,  /*KAL_FALSE for disable auto flicker, KAL_TRUE for enable auto flicker*/
	.test_pattern = KAL_FALSE,        /*KAL_FALSE for in test pattern mode, KAL_TRUE for normal output*/
...
static kal_uint16 set_gain(kal_uint16 gain)
{
	...
	if (gain >= BASEGAIN && gain <= 16*BASEGAIN) {
		...
		if (iReg <= 0x10) {
			...
			write_cmos_sensor(0x01, 0x01);
			LOG_INF("SP2609MIPI_SetGain = 16");
		} else if (iReg >= 0xf8)/*huazai*/ {
			write_cmos_sensor(0xfd, 0x01);
			write_cmos_sensor(0x24, 0xf8);
			write_cmos_sensor(0x01, 0x01);
			LOG_INF("SP2609MIPI_SetGain = 160");
		} else {
			write_cmos_sensor(0xfd, 0x01);
			write_cmos_sensor(0x24, (kal_uint8)iReg);
			write_cmos_sensor(0x01, 0x01);
			LOG_INF("sp260911111-------SP2609MIPI_SetGain ireg = 0x%x,gain = 0x%x\n", iReg, gain);
		}
	} else {
...
static void sensor_init(void)
{
	...
	write_cmos_sensor(0x13, 0x20);
	write_cmos_sensor(0x16, 0x18);
	write_cmos_sensor(0x1f, 0xf7);
	write_cmos_sensor(0x20, 0x07);
	write_cmos_sensor(0x21, 0xa7);
	write_cmos_sensor(0x22, 0x19);
	write_cmos_sensor(0x24, 0xff);
	write_cmos_sensor(0x25, 0x01);
	write_cmos_sensor(0x27, 0x22);
	write_cmos_sensor(0x29, 0xde);
	write_cmos_sensor(0x2a, 0x49);
	write_cmos_sensor(0x2c, 0x70);
	write_cmos_sensor(0x30, 0x03);
	write_cmos_sensor(0x31, 0x01);
	write_cmos_sensor(0x33, 0x40);
	write_cmos_sensor(0x46, 0x03);
	write_cmos_sensor(0x47, 0xfe);
	write_cmos_sensor(0x49, 0x24);
	write_cmos_sensor(0x51, 0x61);
	write_cmos_sensor(0x52, 0x60);
	write_cmos_sensor(0x55, 0x20);
	write_cmos_sensor(0x56, 0x01);
	write_cmos_sensor(0x58, 0x60);
	write_cmos_sensor(0x5d, 0x00);
	write_cmos_sensor(0x5e, 0x15);
	write_cmos_sensor(0x66, 0x8a);
	write_cmos_sensor(0x68, 0x80);
	write_cmos_sensor(0x71, 0xa0);
	write_cmos_sensor(0x72, 0x1a);
	write_cmos_sensor(0x74, 0x90);
	write_cmos_sensor(0x75, 0x1a);
	write_cmos_sensor(0x8a, 0x33);
	write_cmos_sensor(0x8b, 0x33);
	...
	write_cmos_sensor(0xf6, 0x80);
	write_cmos_sensor(0xf7, 0x80);
	write_cmos_sensor(0xfb, 0x7b);
	write_cmos_sensor(0xfc, 0x7f);
	write_cmos_sensor(0xfe, 0x7f);
	write_cmos_sensor(0x01, 0x01);
	write_cmos_sensor(0xfd, 0x02);
	write_cmos_sensor(0x03, 0x7f);
	write_cmos_sensor(0xbc, 0x40);
	write_cmos_sensor(0xbd, 0x30);
	write_cmos_sensor(0xbe, 0x6f);
	write_cmos_sensor(0xbf, 0x0b);
	write_cmos_sensor(0xc5, 0x02);
	write_cmos_sensor(0xc6, 0xdf);
	write_cmos_sensor(0xc7, 0x02);
	write_cmos_sensor(0xc8, 0xdd);
	write_cmos_sensor(0xcb, 0x6f);
	write_cmos_sensor(0xcc, 0x0b);
	write_cmos_sensor(0x04, 0x11);
	write_cmos_sensor(0x05, 0x11);
	write_cmos_sensor(0x08, 0x80);
	write_cmos_sensor(0x09, 0x10);
	write_cmos_sensor(0x0a, 0x80);
	write_cmos_sensor(0x0b, 0x10);
	write_cmos_sensor(0x0c, 0xff);
	write_cmos_sensor(0x0d, 0x00);
	write_cmos_sensor(0x0e, 0x0c);
	write_cmos_sensor(0x0f, 0x17);
	write_cmos_sensor(0x11, 0x14);
	write_cmos_sensor(0x12, 0x9f);
	write_cmos_sensor(0xfd, 0x01);
}    /*    MIPI_sensor_Init  */
...
static kal_uint32 open(void)
{
	...
	/*sensor have two i2c address 0x6c & 0x20, we should detect the module used i2c address*/
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		spin_lock(&imgsensor_drv_lock);
		imgsensor.i2c_write_id = imgsensor_info.i2c_addr_table[i];
		spin_unlock(&imgsensor_drv_lock);
		...
		sensor_id = return_sensor_id();
		if (sensor_id == imgsensor_info.sensor_id) {
			LOG_INF("Expect id: 0x%x,read sensor id: 0x%x\n", imgsensor.i2c_write_id, sensor_id);
			break;
		}
		LOG_INF("Read sensor id fail, Expect id:0x%x, read sensor id: 0x%x\n",
				imgsensor.i2c_write_id, sensor_id);
		retry--;
		} while (retry > 0);
		i++;
					...
					MUINT32 framerate)
{
	...
	case MSDK_SCENARIO_ID_CAMERA_PREVIEW:
			...
			imgsensor.dummy_line =
				(frame_length > imgsensor_info.pre.framelength) ? (frame_length -
	...
	case MSDK_SCENARIO_ID_VIDEO_PREVIEW:
			...
			spin_lock(&imgsensor_drv_lock);
			imgsensor.dummy_line =
				(frame_length > imgsensor_info.normal_video.framelength) ? (frame_length -
	...
	case MSDK_SCENARIO_ID_CAMERA_CAPTURE_JPEG:
			...
			spin_lock(&imgsensor_drv_lock);
			imgsensor.dummy_line =
				(frame_length > imgsensor_info.cap.framelength) ? (frame_length -
	...
	case MSDK_SCENARIO_ID_SLIM_VIDEO:
			...
			spin_lock(&imgsensor_drv_lock);
			imgsensor.dummy_line =
				(frame_length > imgsensor_info.slim_video.framelength) ? (frame_length -
...
UINT32 SP2609_MIPI_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
	/* To Do : Check Sensor status here */
	if (pfFunc != NULL)
		*pfFunc = &sensor_func;
	return ERROR_NONE;
}    /*    SP2609_MIPI_SensorInit    */
