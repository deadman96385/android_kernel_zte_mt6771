...
static int icm206xx_SetPowerMode(struct i2c_client *client, int sensor_type, bool enable)
{
	...
	if (res < 0) {
		ACC_PR_ERR("set power mode failed!\n");
		return ICM206XX_ERR_I2C;
	}
...
static int icm206xx_EnableSensor(struct i2c_client *client, int sensor_type, bool enable)
{
	...
	if (res < 0) {
		ACC_PR_ERR("read power ctl2 register err!\n");
		return ICM206XX_ERR_I2C;
	}
	...
	if (res < 0) {
		ACC_PR_ERR("set power ctl2 failed!\n");
		return ICM206XX_ERR_I2C;
	}
	mdelay(startup_time);
	ACC_LOG("Exit %s enable:%d!\n", __func__, enable ? 1 : 0);
	return ICM206XX_SUCCESS;
}
...
static int icm206xx_ReadChipInfo(struct i2c_client *client, char *buf, int bufsize)
{
	...
	case ICM20608D_WHO_AM_I:
		snprintf(buf, PAGE_SIZE, "ICM20608D [0x%x]", databuf[0]);
		break;
	default:
		snprintf(buf, PAGE_SIZE, "Unknown Sensor [0x%x]", databuf[0]);
		break;
	}
...
{
	u8 databuf[2] = {0};
	u64 delay_time = delay_ns;
	int sample_rate = 0;
	int rate_div = 0;
	int res = 0;
	int i, highest_sample_rate = 0;

	ACC_LOG("Enter %s, delay_ns: %lld\n", __func__, delay_ns);
	/* ---------------------------------------------------- */
	...
	do_div(delay_time, 1000);
	/*sample_rate = (int)(sample_rate / 1000);	 us to ms */
	do_div(delay_time, 1000);
	sample_rate = (int)delay_time;
	if (sample_rate != 0)
	...
	databuf[0] = rate_div;
	ACC_LOG("%s, rate_div:%d sample_rate:%d highest_sample_rate:%d\n",
		__func__, rate_div, sample_rate, highest_sample_rate);
	res = icm206xx_share_i2c_write_register(ICM206XX_REG_SAMRT_DIV, databuf, 1);
	if (res < 0) {
		ACC_PR_ERR("write sample rate register err!\n");
		return ICM206XX_ERR_I2C;
	}
	...
	if (res < 0) {
		ACC_PR_ERR("read accel sample rate register err!\n");
		return ICM206XX_ERR_I2C;
	}

	ACC_LOG("exit %s, read accel sample rate: 0x%x\n", __func__, databuf[0]);
