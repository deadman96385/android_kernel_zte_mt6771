...
static kal_bool GC5025DuringTestPattern = KAL_FALSE;

static kal_uint32 Dgain_ratio = 1;
static kal_bool DR_State = KAL_TRUE;

static imgsensor_info_struct imgsensor_info = {
	.sensor_id = GC5025_SENSOR_ID,        /*record sensor id defined in Kd_imgsensor.h*/

	.checksum_value = 0x56d1c29c,        /*checksum value for Camera Auto Test*/
...
static imgsensor_struct imgsensor = {
	...
	.current_fps = 300,  /* 24fps for PIP, 30fps for Normal or ZSD*/
...
static void gc5025_gcore_read_otp_info(void)
{
	...
	case 0x01:
		...
		if ((check % 255 + 1) == ddchecksum) {
			LOG_INF("GC5025_OTP_DD:DD checksum correct!checksum = 0x%x\n", ddchecksum);
		} else {
		...
		for (i = 0; i < total_number; i++) {
			...
			if (ddtempbuff[4 * i + 3] & 0x10) {
				...
				default:
					...
					gc5025_otp_info.dd_param_type[cnt++] =
	...
	for (i = 0; i < gc5025_otp_info.dd_cnt; i++) {
		if (gc5025_otp_info.dd_param_type[i] == 0) {
			gc5025_otp_info.dd_param_x[i] =
				WINDOW_WIDTH-gc5025_otp_info.dd_param_x[i]+1;
		} else if (gc5025_otp_info.dd_param_type[i] == 1) {
			gc5025_otp_info.dd_param_x[i] =
				WINDOW_WIDTH - gc5025_otp_info.dd_param_x[i]-1;
		} else {
			gc5025_otp_info.dd_param_x[i] =
				WINDOW_WIDTH - gc5025_otp_info.dd_param_x[i];
		}
	}
	#elif defined(GC5025_MIRROR_V)
	for (i = 0; i < gc5025_otp_info.dd_cnt; i++) {
		gc5025_otp_info.dd_param_y[i] =
			WINDOW_HEIGHT - gc5025_otp_info.dd_param_y[i] + 1;
	}
	#elif defined(GC5025_MIRROR_HV)
	for (i = 0; i < gc5025_otp_info.dd_cnt; i++) {
		if (gc5025_otp_info.dd_param_type[i] == 0) {
	...
	for (i = 0 ; i < gc5025_otp_info.dd_cnt-1; i++) {
		...
		for (i = 0; i < gc5025_otp_info.dd_cnt; i++) {
			if (gc5025_otp_info.dd_param_y[i] == gc5025_otp_info.dd_param_y[i+1]) {
				s = i++;
				while ((gc5025_otp_info.dd_param_y[s] == gc5025_otp_info.dd_param_y[i+1])
				...
				for (n = s; n < e; n++) {
					for (m = n+1; m < e+1; m++) {
						if (gc5025_otp_info.dd_param_x[n] > gc5025_otp_info.dd_param_x[m]) {
							temp_x = gc5025_otp_info.dd_param_x[n];
							gc5025_otp_info.dd_param_x[n] = gc5025_otp_info.dd_param_x[m];
							gc5025_otp_info.dd_param_x[m] = temp_x;
							temp_y = gc5025_otp_info.dd_param_y[n];
							gc5025_otp_info.dd_param_y[n] = gc5025_otp_info.dd_param_y[m];
							gc5025_otp_info.dd_param_y[m] = temp_y;
							temp_type = gc5025_otp_info.dd_param_type[n];
							gc5025_otp_info.dd_param_type[n] =
								gc5025_otp_info.dd_param_type[m];
							gc5025_otp_info.dd_param_type[m] = temp_type;
						}
					}
				}
...
static void set_shutter(kal_uint16 shutter)
{
	...
	shutter =
		...
		imgsensor_info.margin)) ? (imgsensor_info.max_frame_length - imgsensor_info.margin) : shutter;
...
static void sensor_init(void)
{
	...
	write_cmos_sensor(0x08, 0x20);/*08->20  20171025*/
	...
	/*MIPI*/
	write_cmos_sensor(0xfe, 0x03);
	write_cmos_sensor(0x02, 0x05);/*01  20170512 modify*/
					...
					MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	/*if (imgsensor.current_fps == imgsensor_info.cap1.max_framerate) {
		...
		if (imgsensor.current_fps != imgsensor_info.cap.max_framerate)
			pr_err("Warning: current_fps %d fps is not support, so use cap's setting: %d fps!\n",
...
static kal_uint32 normal_video(MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT *image_window,
						MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	LOG_INF("GC5025 normal video Entre\n");
							...
							MUINT32 framerate)
{
	...
	case MSDK_SCENARIO_ID_CAMERA_CAPTURE_JPEG:
			/*if (imgsensor.current_fps == imgsensor_info.cap1.max_framerate) {
				...
				spin_lock(&imgsensor_drv_lock);
				imgsensor.dummy_line =
					(frame_length > imgsensor_info.cap1.framelength) ? (frame_length -
				...
				if (imgsensor.current_fps != imgsensor_info.cap.max_framerate)
					LOG_INF("current_fps %d fps is not support, so use cap's setting: %d fps!\n",
