	...
	if (type == UPGRADE_IRAM) {
		if (tfd.hex_tag == BLOCK_TAG_AF) {
			...
			fbi[DATA].mem_start = (fbi[DATA].fix_mem_start != INT_MAX)
				? fbi[DATA].fix_mem_start : DLM_START_ADDRESS;
			fbi[TUNING].mem_start = (fbi[TUNING].fix_mem_start != INT_MAX)
				? fbi[TUNING].fix_mem_start : fbi[DATA].mem_start + fbi[DATA].len;
			fbi[MP].mem_start = (fbi[MP].fix_mem_start != INT_MAX) ? fbi[MP].fix_mem_start :  0;
			fbi[GESTURE].mem_start =
				(fbi[GESTURE].fix_mem_start != INT_MAX) ? fbi[GESTURE].fix_mem_start : 0;
		...
		/* Copy gesture data */
		if (fbi[GESTURE].mem_start != 0xffffffff && ges_fw_start != 0xffffffff
			&& fbi[GESTURE].mem_start != 0 && ges_fw_start != 0)
...
static int ilitek_tddi_fw_ili_convert(u8 *pfw)
{
	...
	if (sizeof(CTPM_FW) < ILI_FILE_HEADER) {
		ipio_err("%s CTPM_FW's size(%d) is too small\n", sizeof(CTPM_FW));
		return -EINVAL;
	}
	...
	for (i = 0; i < FW_BLOCK_INFO_NUM; i++) {
		if (((block_enable >> i) & 0x01) == 0x01) {
			...
			if ((num) == 6) {
				...
				fbi[num].start = (CTPM_FW[34 + i * 6] << 16) + (CTPM_FW[35 + i * 6] << 8)
					+ (CTPM_FW[36 + i * 6]);
				fbi[num].end = (CTPM_FW[37 + i * 6] << 16) + (CTPM_FW[38 + i * 6] << 8)
					+ (CTPM_FW[39 + i * 6]);
				fbi[num].fix_mem_start = INT_MAX;
			}
