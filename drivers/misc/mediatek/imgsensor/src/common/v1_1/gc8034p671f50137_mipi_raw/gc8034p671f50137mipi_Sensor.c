...
static SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[5] = {
	...
	{ 3264, 2448, 0, 0, 3264, 2448, 3264, 2448, 0, 0, 1632, 1224, 0, 0, 1632, 1224}, /* hs video */
...
static void gc8034_gcore_read_otp_info(void)
{
	...
	case 0x01:
		LOG_INF("DD is valid!\n");
		total_number = gc8034_read_otp(0, 0x0c) + gc8034_read_otp(0, 0x0d);
		gc8034_otp_info.dd_flag = 0x01;
		gc8034_read_otp_kgroup(0, 0x0e, &ddtempbuff[0], 4 * total_number);
		for (i = 0; i < total_number; i++) {
			if ((ddtempbuff[4 * i + 3] & 0x80) == 0x80) {
				if ((ddtempbuff[4 * i + 3] & 0x03) == 0x03) {
					gc8034_otp_info.dd_param[cnt].x =
						(((kal_uint16)ddtempbuff[4 * i + 1] & 0x0f) << 8) + ddtempbuff[4 * i];
					gc8034_otp_info.dd_param[cnt].y =
	...
	LOG_INF("flag_module = 0x%x, flag_wb = 0x%x, flag_af = 0x%x\n", flag_module, flag_wb, flag_af);
	...
	for (index = 0; index < 2; index++) {
		...
		case 0x00:
			LOG_INF("wb current group %d is empty!\n", index + 1);
			gc8034_otp_info.wb_flag = gc8034_otp_info.wb_flag | 0x00;
			break;
		case 0x04:
			LOG_INF("wb current group %d is valid!\n", index + 1);
			check = 0;
			...
			if ((check % 255 + 1) == wb[WB_WIDTH - 1]) {
				...
				LOG_INF("wb current checksum %d error!\n", index + 1);
			break;
		case 0x08:
		case 0x0c:
			LOG_INF("wb current group %d is invalid!\n", index + 1);
	...
	for (index = 0; index < 2; index++) {
		...
		case 0x04:
			...
			check = 0;
			if (index == 0) {
				gc8034_read_otp_kgroup(3, 0x44, &gc8034_otp_info.lsc_param[0], 396);
				lsc_checksum = gc8034_read_otp(6, 0x50);
			} else {
				gc8034_read_otp_kgroup(6, 0x51, &gc8034_otp_info.lsc_param[0], 396);
			...
			if ((check % 255 + 1) == lsc_checksum)
				...
				LOG_INF("lsc checksum error, checksum read: 0x%x, calculate: 0x%x\n",
...
static void gc8034_otp_identify(void)
{
...
static void gc8034_otp_function(void)
{
	...
	write_cmos_sensor(0xf2, 0x01);
	write_cmos_sensor(0xf4, 0x88);
	gc8034_read_otp_group(0, GC8034_OTP_ID_DATA_OFFSET, &otp_id[0], GC8034_OTP_ID_SIZE);
	...
	if (flag == 1) {
		...
		memset(&gc8034_otp_info, 0, sizeof(gc8034_otp_info));
		for (i = 0; i < GC8034_OTP_ID_SIZE; i++)
			gc8034_otp_info.otp_id[i] = otp_id[i];
		gc8034_gcore_read_otp_info();
	}
...
static void gain2reg(kal_uint16 gain)
{
	...
	kal_uint8 agc_register[2 * MAX_AG_INDEX][AGC_REG_NUM] = {
	  /*{ 0xfe, 0x20, 0x33, 0xd0, 0xfe, 0xdf, 0xe7, 0xe8, 0xe9, 0xea, 0xeb, 0xec, 0xed, 0xee, 0xfe },*/
		...
		{ 0x00, 0x52, 0x80, 0x01, 0x0c, 0x35, 0x3a, 0x2b, 0x2d, 0x4c, 0x67, 0x8d, 0xc0, 0x00 },
		/* fullsize */
		{ 0x00, 0x54, 0x82, 0x01, 0x06, 0x18, 0x20, 0x16, 0x17, 0x50, 0x6c, 0x9b, 0xd8, 0x00 },
		{ 0x00, 0x54, 0x82, 0x01, 0x06, 0x18, 0x20, 0x16, 0x17, 0x50, 0x6c, 0x9b, 0xd8, 0x00 },
		{ 0x00, 0x4e, 0x83, 0x01, 0x0c, 0x2e, 0x2d, 0x15, 0x19, 0x47, 0x70, 0x9f, 0xd8, 0x00 },
		{ 0x00, 0x51, 0x80, 0x01, 0x07, 0x28, 0x32, 0x22, 0x20, 0x49, 0x70, 0x91, 0xd9, 0x00 },
		{ 0x00, 0x4c, 0x82, 0x01, 0x0f, 0x3b, 0x3b, 0x1c, 0x1f, 0x47, 0x6f, 0x9b, 0xd3, 0x00 },
		{ 0x00, 0x4c, 0x82, 0x01, 0x0f, 0x3b, 0x3b, 0x1c, 0x1f, 0x47, 0x6f, 0x9b, 0xd3, 0x00 },
		{ 0x00, 0x51, 0x7f, 0x01, 0x0c, 0x35, 0x3a, 0x2b, 0x2d, 0x4c, 0x67, 0x8d, 0xc0, 0x00 },
		{ 0x00, 0x53, 0x7f, 0x01, 0x0c, 0x35, 0x3a, 0x2b, 0x2d, 0x4c, 0x67, 0x8d, 0xc0, 0x00 },
		{ 0x00, 0x54, 0x7e, 0x01, 0x0c, 0x35, 0x3a, 0x2b, 0x2d, 0x4c, 0x67, 0x8d, 0xc0, 0x00 }
	};
	...
	for (gain_index = MEAG_INDEX - 1; gain_index >= 0; gain_index--)
		if (gain >= gain_level[gain_index]) {
			...
			write_cmos_sensor(0x20, agc_register[BorF * MAX_AG_INDEX + gain_index][1]);
			write_cmos_sensor(0x33, agc_register[BorF * MAX_AG_INDEX + gain_index][2]);
			write_cmos_sensor(0xfe, agc_register[BorF * MAX_AG_INDEX + gain_index][3]);
			write_cmos_sensor(0xdf, agc_register[BorF * MAX_AG_INDEX + gain_index][4]);
			write_cmos_sensor(0xe7, agc_register[BorF * MAX_AG_INDEX + gain_index][5]);
			write_cmos_sensor(0xe8, agc_register[BorF * MAX_AG_INDEX + gain_index][6]);
			write_cmos_sensor(0xe9, agc_register[BorF * MAX_AG_INDEX + gain_index][7]);
			write_cmos_sensor(0xea, agc_register[BorF * MAX_AG_INDEX + gain_index][8]);
			write_cmos_sensor(0xeb, agc_register[BorF * MAX_AG_INDEX + gain_index][9]);
			write_cmos_sensor(0xec, agc_register[BorF * MAX_AG_INDEX + gain_index][10]);
			write_cmos_sensor(0xed, agc_register[BorF * MAX_AG_INDEX + gain_index][11]);
			write_cmos_sensor(0xee, agc_register[BorF * MAX_AG_INDEX + gain_index][12]);
			write_cmos_sensor(0xfe, agc_register[BorF * MAX_AG_INDEX + gain_index][13]);
			break;
		}
}
...
static void sensor_init(void)
{
	...
	write_cmos_sensor(0x92, GC8034_FULL_STARTY);
	write_cmos_sensor(0x94, GC8034_FULL_STARTX);
...
static void binning_setting(void)
{
	...
	/* Crop window */
	write_cmos_sensor(0x90, 0x01);
	write_cmos_sensor(0x92, GC8034_BIN_STARTY);
	write_cmos_sensor(0x94, GC8034_BIN_STARTX);
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			*sensor_id = return_sensor_id();
			...
			pr_debug("[gc8034_camera_sensor]get_imgsensor_id:Read sensor id fail, write id: 0x%x, id: 0x%x\n",
				imgsensor.i2c_write_id, *sensor_id);
			retry--;
		} while (retry > 0);
		i++;
...
static kal_uint32 open(void)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			sensor_id = return_sensor_id();
			if (sensor_id == imgsensor_info.sensor_id) {
				pr_debug("[gc8034_camera_sensor]open:i2c write id: 0x%x, sensor id: 0x%x\n",
					imgsensor.i2c_write_id, sensor_id);
				break;
			}
			pr_debug("[gc8034_camera_sensor]open:Read sensor id fail, write id: 0x%x, id: 0x%x\n",
				imgsensor.i2c_write_id, sensor_id);
			retry--;
		} while (retry > 0);
		i++;
...
UINT32 GC8034P671F50137_MIPI_RAW_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
