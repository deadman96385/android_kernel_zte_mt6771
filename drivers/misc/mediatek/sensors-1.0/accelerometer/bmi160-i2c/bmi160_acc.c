...
#endif
static s32 i2c_dma_read(struct i2c_client *client,
		...
		u8 addr, u8 *data, u8 len)
{
	...
	if (err != 2) {
		GSE_ERR("i2c_transfer error: %x %x (%d %p %d) %d\n",
...
static int bmi160_acc_set_range(struct i2c_client *client, unsigned char range)
{
	...
	if (client == NULL)
		return -1;
	mutex_lock(&obj->lock);
	comres = bma_i2c_read_block(client, BMI160_USER_ACC_RANGE__REG, data + 1, 1);
...
static int bmi160_fifo_length(uint32_t *fifo_length)
{
	...
	comres +=
	    bma_i2c_read_block(client, BMI160_USER_FIFO_BYTE_COUNTER_LSB__REG, a_data_u8r, 2);
...
static int bmi160acc_setup_eint(struct i2c_client *client)
{
	...
	if (node) {
		...
		if (request_irq
		    (obj->IRQ, bmi_irq_handler, IRQF_TRIGGER_RISING, "mediatek,GSE_1-eint", NULL)) {
...
static int bmi160_acc_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (err) {
		GSE_ERR("bmi160_factory_device register failed.\n");
...
static int bmi160_acc_i2c_remove(struct i2c_client *client)
{
	...
	/* misc_deregister(&bmi160_acc_device); */
	accel_factory_device_deregister(&bmi160_factory_device);
	GSE_LOG("bmi160_factory_device deregister\n");
