...
#define UBL_I2C_ADDR 0x70

#define SENSOR_MAX_X 720
#define SENSOR_MAX_Y 1440
...
#endif
unsigned int ldo_gpio_number = 0;
unsigned int rst_gpio_number = 0;
unsigned int int_gpio_number = 0;
...
#if 1// CONFIG_OF_TOUCH
static int of_get_synaptic_platform_data(struct device *dev)
{
	TPD_DMESG("enter get_synaptic_platform_data\n");
	...
	rst_gpio_number = of_get_named_gpio(dev->of_node, "rst-gpio", 0);
	int_gpio_number = of_get_named_gpio(dev->of_node, "int-gpio", 0);
	ldo_gpio_number = of_get_named_gpio(dev->of_node, "ldo-gpio", 0);

	TPD_DMESG("rst_gpio_number %d\n", rst_gpio_number);
	TPD_DMESG("int_gpio_number %d\n", int_gpio_number);
	TPD_DMESG("ldo_gpio_number %d\n", ldo_gpio_number);
...
static int tpd_irq_registration(void)
{
	...
	if (node) {
		...
		ret = request_irq(touch_irq, (irq_handler_t) tpd_eint_handler, IRQF_TRIGGER_LOW,
...
{
    struct synaptics_rmi4_data *rmi4_data = (struct synaptics_rmi4_data*) cdev->private;
    strcpy(cdev->ic_tpinfo.tp_name, "Synaptics");
    cdev->ic_tpinfo.chip_model_id = 4;
    cdev->ic_tpinfo.chip_part_id= 0;
    cdev->ic_tpinfo.module_id= (rmi4_data->config_id /65536)%256;
    cdev->ic_tpinfo.chip_ver = 0;
    cdev->ic_tpinfo.firmware_ver= rmi4_data->config_id %65536;
    cdev->ic_tpinfo.i2c_type = 0;
    cdev->ic_tpinfo.i2c_addr = rmi4_data->i2c_client->addr;
    return 0;
}
...
#endif
{
	...
	TPD_DMESG("focal check lcm name = %s", tmp_name);
	
	if(NULL != strstr(tmp_name, "td4100")) 
	{
...
#if 0	// !defined CONFIG_MTK_LEGACY// add 3.3v begin
	tpd->tpd_dev->of_node = of_find_compatible_node(NULL, NULL, "mediatek,synaptics_dsx");
	if (tpd->tpd_dev->of_node)
	{
		printk("of_find_compatible_node-->mediatek,mt6797-touch\n");

		tpd->reg = regulator_get(tpd->tpd_dev, "vmch");
		ret = regulator_set_voltage(tpd->reg, 3300000, 3300000);	/*set 3.3v*/
		if (ret) {
...
//	msleep(DELAY_UI_READY);
	...
	/* set INT mode */
//	gpio_request_one(int_gpio_number, GPIOF_IN,"tpd_int");

//	gpio_direction_input(int_gpio_number);

	/* set RST mode */

//	gpio_request_one(rst_gpio_number, GPIOF_OUT_INIT_HIGH,"touchp_reset");

//	msleep(DELAY_BOOT_READY);
	
//	gpio_direction_output(rst_gpio_number, 0);
...
//	TPD_DMESG("init timing set done\n");
		...
		TPD_DMESG("not synaptic tp,quit .\n");
	}
...
//#endif
	...
	if(client->addr != 0x22){
		TPD_DMESG("synaptic i2c addr = 0x%x!\n",client->addr);
		client->addr=0x22;
	}
		...
		bool enable)
{
	...
	TPD_DMESG("synaptic synaptics_rmi4_f11_wg!\n");
		...
		bool enable)
{
	...
	rmi = &(rmi4_data->rmi4_mod_info);
	TPD_DMESG("synaptic synaptics_rmi4_f12_wg!\n");
...
static int tpd_local_init(void)
{
	...
	TPD_DMESG("Synaptics local init\n");
