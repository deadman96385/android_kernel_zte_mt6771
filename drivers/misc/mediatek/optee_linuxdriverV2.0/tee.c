...
struct wake_lock optee_os_wk_lock;
			       ...
			       struct tee_ioctl_shm_alloc_data __user *udata)
{
	...
	/*pr_info(" chenlu tee_ioctl_shm_alloc 0726-2\n");*/

	if (copy_from_user(&data, udata, sizeof(data))) {
		pr_info(" lllllllllllll tee_ioctl_shm_alloc copy_from_user failed\n");
		return -EFAULT;
	}

	/* Currently no input flags are supported */
	if (data.flags) {
		pr_info(" lllllllllllll tee_ioctl_shm_alloc data.flags failed\n");
		return -EINVAL;
	}

	data.id = -1;

	/*pr_info("\nchenlu data.size = %llx\n", data.size);*/

	shm = tee_shm_alloc(ctx->teedev, data.size,
			TEE_SHM_MAPPED | TEE_SHM_DMA_BUF);
	if (IS_ERR(shm)) {
		pr_info("tee_ioctl_shm_alloc IS_ERR(shm):%ld\n", PTR_ERR(shm));
		return PTR_ERR(shm);
	}
	...
	/*pr_info("\nchenlu data.id = %d, data.flags = %d, data.size = %llx\n", data.id, data.flags,
			data.size); */

	if (copy_to_user(udata, &data, sizeof(data))) {
		pr_info(" lllllllllllll tee_ioctl_shm_alloc copy_from_user failed 2\n");
		ret = -EFAULT;
	} else {
	...
	/*pr_info("\nchenlu tee_ioctl_shm_alloc ret = %lx\n", ret); */
			...
			struct tee_ioctl_buf_data __user *ubuf)
{
	...
	if (!ctx->teedev->desc->ops->invoke_func) {
		pr_info("llllllllllllllllllllllll  tee_ioctl_invoke EINVAL\n");
		return -EINVAL;
	}

	rc = copy_from_user(&buf, ubuf, sizeof(buf));
	if (rc) {
		pr_info("llllllllllllllllllllllll  tee_ioctl_invoke copy_from_user fail\n");
		return rc;
	}

	if (buf.buf_len > TEE_MAX_ARG_SIZE ||
		buf.buf_len < sizeof(struct tee_ioctl_invoke_arg)) {
		pr_info("llllllllllllllllllllllll  tee_ioctl_invoke EINVAL 2\n");
		return -EINVAL;
	}

	uarg = (struct tee_ioctl_invoke_arg __user *)(unsigned long)buf.buf_ptr;
	if (copy_from_user(&arg, uarg, sizeof(arg))) {
		pr_info("llllllllllllllllllllllll  tee_ioctl_invoke EFAULT 2\n");
		return -EFAULT;
	}

	if (sizeof(arg) + TEE_IOCTL_PARAM_SIZE(arg.num_params) != buf.buf_len) {
		pr_info("llllllllllllllllllllllll  tee_ioctl_invoke EINVAL 3\n");
		return -EINVAL;
	}
	...
	if (arg.num_params) {
		params = kcalloc(arg.num_params, sizeof(struct tee_param),
				 GFP_KERNEL);
		if (!params) {
			pr_info("llllllllllllllllllllllll  tee_ioctl_invoke kcalloc ENOMEM\n");
			return -ENOMEM;
		}
		uparams = (struct tee_ioctl_param __user *)(uarg + 1);
		rc = params_from_user(ctx, params, arg.num_params, uparams);
		if (rc) {
			pr_info("llllllllllllllllllllllll  tee_ioctl_invoke params_from_user fail 2\n");
			goto out;
		}
	}
	...
	if (rc) {
		pr_info("llllllllllllllllllllllll  tee_ioctl_invoke invoke_func fail\n");
		goto out;
	}
	...
	if (put_user(arg.ret, &uarg->ret) ||
		put_user(arg.ret_origin, &uarg->ret_origin)) {
		rc = -EFAULT;
		pr_info("llllllllllllllllllllllll  tee_ioctl_invoke EFAULT 3\n");
		goto out;
	}
	rc = params_to_user(uparams, arg.num_params, params);
out:
	if (params) {
       ...
       /*pr_info("llllllllllllllllllllllll  tee_ioctl_invoke end return %d\n", rc);*/
	return rc;
}
...
static long tee_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case TEE_IOC_OPEN_SESSION:
		...
		/*pr_info("wake_lock(&optee_os_wk_lock) for TEE_IOC_OPEN_SESSION!");*/
		retValue = tee_ioctl_open_session(ctx, uarg);
		/*wake_unlock(&optee_os_wk_lock);*/
		/*pr_info("wake_unlock(&optee_os_wk_lock) for TEE_IOC_OPEN_SESSION!");*/
		return retValue;
	case TEE_IOC_INVOKE:
		/*wake_lock(&optee_os_wk_lock);*/
		/*pr_info("wake_lock(&optee_os_wk_lock) for TEE_IOC_INVOKE!");	*/
		retValue = tee_ioctl_invoke(ctx, uarg);
		/*wake_unlock(&optee_os_wk_lock);*/
		/*pr_info("wake_unlock(&optee_os_wk_lock) for TEE_IOC_INVOKE!");*/
		/*pr_info("llllllllllllllllll TEE_IOC_INVOKE return %ld\n", retValue);*/
		return retValue;
	case TEE_IOC_CANCEL:
		/*wake_lock(&optee_os_wk_lock);*/
		/*pr_info("wake_lock(&optee_os_wk_lock) for TEE_IOC_CANCEL!");	*/
		retValue = tee_ioctl_cancel(ctx, uarg);
		/*wake_unlock(&optee_os_wk_lock);*/
		/*pr_info("wake_unlock(&optee_os_wk_lock) for TEE_IOC_CANCEL!");*/
		return retValue;
	case TEE_IOC_CLOSE_SESSION:
		/*wake_lock(&optee_os_wk_lock);*/
		/*pr_info("wake_lock(&optee_os_wk_lock) for TEE_IOC_CLOSE_SESSION!");	*/
		retValue = tee_ioctl_close_session(ctx, uarg);
		/*wake_unlock(&optee_os_wk_lock);*/
		/*pr_info("wake_unlock(&optee_os_wk_lock) for TEE_IOC_CLOSE_SESSION!");*/
		return retValue;
...
static int __init tee_init(void)
{
	...
	/*pr_info("\ntee_init 0728,optee_os_wk_lock wake_lock_init\n"); */
	wake_lock_init(&optee_os_wk_lock, WAKE_LOCK_SUSPEND, "optee_os_wakelock");
	return rc;
}
