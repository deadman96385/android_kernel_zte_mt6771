...
#else
#define MIPITX_INREGBIT(addr, field) (REG_FLD_VAL_GET((field), __raw_readl((unsigned long *)(addr))))
...
static void _DSI_INTERNAL_IRQ_Handler(enum DISP_MODULE_ENUM module, unsigned int param)
{
	...
	if (status.TE_RDY) {
		...
		if (txrx_ctrl.EXT_TE_EN == 1)
			wake_up(&(_dsi_context[i].ext_te_wq));
		else
			wake_up(&(_dsi_context[i].bta_te_wq));
...
void DSI_exit_ULPS(enum DISP_MODULE_ENUM module)
{
	...
	for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module); i++) {
		...
		DSI_OUTREGBIT(NULL, struct DSI_TXRX_CTRL_REG, DSI_REG[i]->DSI_TXRX_CTRL, LANE_NUM,
					  lane_num_bitvalue);
...
#if 0
static int _is_lcm_vdo_mode(DISP_MODULE_ENUM module)
{
	int i = 0;
	int ret = 0;
...
int MIPITX_IsEnabled(enum DISP_MODULE_ENUM module, struct cmdqRecStruct *cmdq)
{
	...
	if (module == DISP_MODULE_DSIDUAL) {
		ASSERT(MIPITX_INREGBIT(MIPI_BASE_ADDR(DISP_MODULE_DSI0)+MIPITX_PLL_CON1,
				FLD_RG_DSI_PLL_EN)
				== MIPITX_INREGBIT(MIPI_BASE_ADDR(DISP_MODULE_DSI1) + MIPITX_PLL_CON1,
				FLD_RG_DSI_PLL_EN));
		ret = MIPITX_INREGBIT(MIPI_BASE_ADDR(DISP_MODULE_DSI0)+MIPITX_PLL_CON1,
				FLD_RG_DSI_PLL_EN);
...
unsigned int dsi_phy_get_clk(enum DISP_MODULE_ENUM module)
{
	...
	unsigned int pcw = (DSI_PHY_REG[i]->MIPITX_DSI_PLL_CON1.RG_DSI_MPPLL_SDM_PCW >> 24) & 0x7f;
	unsigned int prediv = (1 << (DSI_PHY_REG[i]->MIPITX_DSI_PLL_CON1.RG_DSI0_MPPLL_PREDIV));
	unsigned int posdiv = (1 << (DSI_PHY_REG[i]->MIPITX_DSI_PLL_CON1.RG_DSI0_MPPLL_POSDIV));
	unsigned int txdiv0 = (1 << (DSI_PHY_REG[i]->MIPITX_DSI_PLL_CON1.RG_DSI_MPPLL_TXDIV0));
	unsigned int txdiv1 = (1 << (DSI_PHY_REG[i]->MIPITX_DSI_PLL_CON1.RG_DSI_MPPLL_TXDIV1));
	unsigned int pll_preserve = (1 << (DSI_PHY_REG[i]->MIPITX_DSI_PLL_CON4.RG_DSI_PLL_RESERVED));

	DISPERR("%s, pcw: %d, prediv: %d, posdiv: %d, txdiv0: %d, txdiv1: %d, preserve: %d\n", __func__, pcw,
	prediv, posdiv, txdiv0, txdiv1, pll_preserve);
...
static void _dsi_basic_irq_enable(enum DISP_MODULE_ENUM module, void *cmdq)
{
	...
	if (module == DISP_MODULE_DSIDUAL) {
		...
		DSI_OUTREGBIT(cmdq, struct DSI_INT_ENABLE_REG, DSI_REG[0]->DSI_INTEN, BUFFER_UNDERRUN_INT_EN, 1);
		DSI_OUTREGBIT(cmdq, struct DSI_INT_ENABLE_REG, DSI_REG[1]->DSI_INTEN, BUFFER_UNDERRUN_INT_EN, 1);
...
int ddp_dsi_switch_mode(enum DISP_MODULE_ENUM module, void *cmdq_handle, void *params)
{
	...
	if (mode == 0) {	/* V2C */
		...
		DISP_REG_SET_FIELD(cmdq_handle, FLD_RG_DSI_PLL_SDM_SSC_EN, DSI_PHY_REG[i]+MIPITX_PLL_CON2, 0);
...
int ddp_dsi_ioctl(enum DISP_MODULE_ENUM module, void *cmdq_handle, enum DDP_IOCTL_NAME ioctl_cmd,
		  void *params)
{
	int ret = 0;
