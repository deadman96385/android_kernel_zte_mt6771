...
struct fts_esdcheck_st
{
    ...
    u8      suspend             : 1;
    u8      enter_into_gesture  : 1;    /* 1- enter into gesture */
...
#if FTS_GESTURE_EN
    /* Only need write gesture register in LCD off state */
    if (fts_esdcheck_data.enter_into_gesture == 1)
    {
        FTS_DEBUG("[%s] recovery gesture register", __func__);
...
static bool get_chip_id( void )
{
    ...
    for (i = 0; i < 3; i++)
    {
        ...
        if ( err < 0 )
        {
            FTS_DEBUG("[%s] Read Reg 0xA3 failed ret = %d\n", __func__, err);
            fts_esdcheck_data.i2c_nack_cnt++;
        }
        else
        {
            if ( (reg_value == fts_updateinfo_curr.CHIP_ID) || (reg_value == 0xEF) ) /* Upgrade sometimes can't detect */
            {
                break;
            }
            else
    ...
    if (i >= 3)
    {
        FTS_DEBUG("[%s] Read Chip id 3 times failed, need execute TP reset.\n", __func__);
        return 1;
    }
...
static bool get_flow_cnt( void )
{
    ...
    if (err < 0)
    {
        FTS_DEBUG("[%s] Read Reg 0x91 failed ret = %d\n", __func__, err);
        fts_esdcheck_data.i2c_nack_cnt++;
    }
    else
    {
    ...
    {
        FTS_DEBUG("[%s] Flow Work Cnt(reg0x91) keep a value for 5 times, need execute TP reset.\n", __func__);
        return 1;
    }
...
static int esdcheck_algorithm(void)
{
    ...
    {
        FTS_DEBUG("[%s] In interrupt state, not check esd, return immediately.\n", __func__);
        return 0;
    }
    ...
    {
        FTS_DEBUG("[%s] In suspend, not check esd, return immediately.\n", __func__);
    ...
    {
        FTS_DEBUG("[%s] In apk or adb command mode, not check esd, return immediately.\n", __func__);
        return 0;
    }
    ...
    else if ( (reg_value & 0x70) ==  FTS_FACTORYMODE_VALUE)
    {
        FTS_DEBUG("[%s] In factory mode, not check esd, return immediately.\n", __func__);
        return 0;
    }
    ...
    FTS_DEBUG("[%s] NoACK=%d, Error Data=%d, Hardware Reset=%d\n", __func__, fts_esdcheck_data.i2c_nack_cnt, fts_esdcheck_data.i2c_dataerror_cnt, fts_esdcheck_data.hardware_reset_cnt);
    return 0;
}
...
*  Return:
*******************************************************************************/
int fts_esdcheck_suspend(bool gesture_en)
{
    FTS_FUNC_ENTER();
    if (gesture_en)
    {
        fts_esdcheck_data.enter_into_gesture = 1;
    }
    else
    {
...
int fts_esdcheck_resume( void )
{
    ...
    fts_esdcheck_data.enter_into_gesture = 0;
    FTS_FUNC_EXIT();
...
int fts_esdcheck_init(void)
{
    ...
    {
        FTS_DEBUG("[%s] Failed to create esd work queue!\n", __func__);
    }
