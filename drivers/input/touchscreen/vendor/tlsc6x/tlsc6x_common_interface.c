...
extern unsigned char g_chip_ver;
#endif
...
#ifdef TLSC_IC_HARD_BATCH
	if (g_chip_ver != TLSC_IC_NG_BATCH) {
		snprintf(cdev->ic_tpinfo.chip_batch, sizeof(cdev->ic_tpinfo.chip_batch), "0x%x", g_chip_ver);
	} else {
		snprintf(cdev->ic_tpinfo.chip_batch, sizeof(cdev->ic_tpinfo.chip_batch), "-1");
	}
#endif
