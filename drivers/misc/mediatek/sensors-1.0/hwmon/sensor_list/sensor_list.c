...
#else
int sensorlist_register_deviceinfo(int sensor, char *devName)
{
	int handle = -1;
	...
	size_t count, loff_t *ptr)
{
	...
	pr_info("sensorlist_read acc name : %s\n", sensor_name);
	if (sensor_name != NULL) {
		sensorlist_register_deviceinfo(ID_ACCELEROMETER, sensor_name);
		atomic_set(&first_ready_after_boot, 1);
	}
#endif

	if (!atomic_read(&first_ready_after_boot)) {
		pr_err("atomic_read fail\n");
		return -EINVAL;
	}
