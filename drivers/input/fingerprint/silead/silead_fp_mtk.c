...
#if (!defined(CONFIG_SILEAD_FP_PLATFORM) && defined(CONFIG_SILEAD_FP_6736))
#include "mtk_spi.h"
...
#endif /* !CONFIG_SILEAD_FP_PLATFORM  6736*/

#define FP_IRQ_OF  "sil,silead_fp-pins"
#define FP_PINS_OF "sil,silead_fp-pins"

const static uint8_t TANAME[] = { 0x51, 0x1E, 0xAD, 0x0D, 0x00, 0x00, 0x00, 0x00,
...
static void silfp_hw_poweron(struct silfp_data *fp_dev)
{
	...
	LOG_MSG_DEBUG(INFO_LOG, "[%s] power supply enter.\n", __func__);
...
static void silfp_hw_reset(struct silfp_data *fp_dev, u8 delay)
{
	...
	ret = pinctrl_select_state(fp_dev->pin.pinctrl, fp_dev->pin.pins_rst_l);
	if (ret) {
		LOG_MSG_DEBUG(ERR_LOG, "[%s] can't set pin reset low\n", __func__);
	}
...
static void silfp_pwdn(struct silfp_data *fp_dev, u8 flag_avdd)
{
	...
	if (flag_avdd == SIFP_PWDN_POWEROFF) {
		silfp_hw_poweroff(fp_dev);
	}
}
...
static int silfp_parse_dts(struct silfp_data *fp_dev)
{
	...
	LOG_MSG_DEBUG(INFO_LOG, "[%s] entry\n", __func__);
...
static int silfp_set_spi(struct silfp_data *fp_dev, bool enable)
{
	...
	if (enable && !atomic_read(&fp_dev->spionoff_count)) {
		LOG_MSG_DEBUG(DBG_LOG, "%s, start to enable spi clk .\n", __func__);
		atomic_inc(&fp_dev->spionoff_count);
		mt_spi_enable_master_clk(fp_dev->spi);    /* FOR MT6797 */
	} else if (atomic_read(&fp_dev->spionoff_count)) {
		LOG_MSG_DEBUG(DBG_LOG, "orenda, [%s] test\n", __func__);
		atomic_dec(&fp_dev->spionoff_count);
		mt_spi_disable_master_clk(fp_dev->spi);    /* FOR MT6797 */
	}
	LOG_MSG_DEBUG(DBG_LOG, "orenda, [%s] done .\n", __func__);
...
static int silfp_resource_init(struct silfp_data *fp_dev, struct fp_dev_init_t *dev_info)
{
	...
	LOG_MSG_DEBUG(INFO_LOG, "[%s] start parse dts\n", __func__);
	...
	*/
	LOG_MSG_DEBUG(DBG_LOG, "[%s] int_port %d, rst_port %d.\n", __func__, fp_dev->int_port, fp_dev->rst_port);
	...
	*/
	...
	memcpy(dev_info->ta, SILEAD_UUID, sizeof(SILEAD_UUID));
