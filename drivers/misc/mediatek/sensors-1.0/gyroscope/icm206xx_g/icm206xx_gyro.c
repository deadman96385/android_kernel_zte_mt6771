...
static int icm206xx_gyro_i2c_resume(struct device *dev);
static int icm206xx_gyro_i2c_suspend(struct device *dev);
...
static const struct dev_pm_ops icm26xx_gyro_pm_ops = {
	SET_SYSTEM_SLEEP_PM_OPS(icm206xx_gyro_i2c_suspend, icm206xx_gyro_i2c_resume)
};
#endif
...
#if defined GYRO_OWN_MISCDEVICE || defined ICM206XX_SELFTEST
...
(struct i2c_client *client, s16 data[ICM206XX_AXIS_NUM])
{
	...
	if (res < 0) {
		GYRO_PR_ERR("ICM206XX read gyro data  error\n");
		return ICM206XX_ERR_I2C;
	}
...
static int icm206xx_gyro_ReadSensorData(struct i2c_client *client, char *buf, int bufsize)
{
	...
	snprintf(buf, PAGE_SIZE, "%04x %04x %04x", data[ICM206XX_AXIS_X],
...
static int icm206xx_gyro_init_client(struct i2c_client *client, bool enable)
{
	...
	res = icm206xx_share_SetSampleRate(ICM206XX_SENSOR_TYPE_GYRO, 2000000, false);
	if (res != ICM206XX_SUCCESS)
		return res;
...
{
	ssize_t len = 0;
	struct gyro_hw *obj = &obj_i2c_data->hw;
...
static int icm206xx_gyro_create_attr(struct device_driver *driver)
{
	int idx;
	int res = 0;
	int num = (int)(ARRAY_SIZE(icm206xx_gyro_attr_list));
...
/* if use  this type of enable , G sensor only enabled but not report inputEvent to HAL */
static int icm206xx_gyro_enable_nodata(int en)
{
	...
	if (res != ICM206XX_SUCCESS) {
		GYRO_PR_ERR("icm206xx_gyro_enable_nodata fail!\n");
		return res;
	}
...
static int icm206xx_gyro_factory_do_self_test(void)
{
	return 0;
}

static int icm206xx_gyro_factory_get_cali(int32_t data[3])
{
	int err;
	struct SENSOR_DATA sensor_data;

	err = icm206xx_gyro_ReadCalibration(icm206xx_gyro_i2c_client, &sensor_data);
	if (err) {
		GYRO_PR_ERR("icm206xx_gyro_ReadCalibration failed!\n");
		return err;
	}
...
static int icm206xx_gyro_factory_set_cali(int32_t data[3])
{
	int err = 0;
	struct SENSOR_DATA sensor_data;
	...
	GYRO_LOG("gyro set cali:[%5d %5d %5d]\n", sensor_data.x, sensor_data.y, sensor_data.z);
	err = icm206xx_gyro_WriteCalibration(icm206xx_gyro_i2c_client, &sensor_data);
	if (err) {
		GYRO_PR_ERR("ICM206xx_gyro_WriteCalibration failed!\n");
		return err;
	}
	return 0;
}

static int icm206xx_gyro_factory_enable_calibration(void)
{
	return 0;
}
static int icm206xx_gyro_factory_clear_cali(void)
{
	int err = 0;

	err = icm206xx_gyro_ResetCalibration(icm206xx_gyro_i2c_client);
	if (err) {
		GYRO_PR_ERR("icm206xx_gyro_factory_clear_cali failed!\n");
		return err;
	}
	return 0;
}

static int icm206xx_gyro_factory_get_raw_data(int32_t data[3])
{
	char strbuf[ICM206XX_BUFSIZE] = { 0 };

	icm206xx_gyro_ReadSensorData(icm206xx_gyro_i2c_client, strbuf, ICM206XX_BUFSIZE);
	GYRO_LOG("support icm206xx_gyro_factory_get_raw_data!\n");
	return 0;
}

static int icm206xx_gyro_factory_get_data(int32_t data[3], int *status)
{
	return icm206xx_gyro_get_data(&data[0], &data[1], &data[2], status);

}

static int icm206xx_gyro_factory_enable_sensor(bool enabledisable, int64_t sample_periods_ms)
{
	int err;

	err = icm206xx_gyro_enable_nodata(enabledisable == true ? 1 : 0);
	if (err) {
		GYRO_PR_ERR("%s enable gyro sensor failed!\n", __func__);
		return err;
	}
	err = icm206xx_gyro_batch(0, sample_periods_ms * 1000000, 0);
	if (err) {
		GYRO_PR_ERR("%s enable Gyro set batch failed!\n", __func__);
		return err;
	}
	return 0;
}

static struct gyro_factory_fops icm206xx_gyro_factory_fops = {
	.enable_sensor = icm206xx_gyro_factory_enable_sensor,
	.get_data = icm206xx_gyro_factory_get_data,
	.get_raw_data = icm206xx_gyro_factory_get_raw_data,
	.enable_calibration = icm206xx_gyro_factory_enable_calibration,
	.clear_cali = icm206xx_gyro_factory_clear_cali,
	.set_cali = icm206xx_gyro_factory_set_cali,
	.get_cali = icm206xx_gyro_factory_get_cali,
	.do_self_test = icm206xx_gyro_factory_do_self_test,
};

static struct gyro_factory_public icm206xx_gyro_factory_device = {
	.gain = 1,
	.sensitivity = 1,
	.fops = &icm206xx_gyro_factory_fops,
};

#ifdef CONFIG_TRAN_SYSTEM_DEVINFO
extern void app_get_gyro_sensor_name(char *gyro_name);
#endif
...
static int icm206xx_gyro_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (icm206xx_accel_i2c_client == NULL) {
		GYRO_PR_ERR("Its not ICM206xx exists.\n");
		res = -EFAULT;
		goto exit;
	}
	...
	res = get_gyro_dts_func(client->dev.of_node, &obj->hw);
	if (res < 0) {
	...
	if (obj->hw.addr != 0) {
		...
		client->addr = obj->hw.i2c_addr[0];  /*i2c addr configed in dts.*/
	}
	...
	res = gyro_factory_device_register(&icm206xx_gyro_factory_device);

	if (res) {
		GYRO_PR_ERR("icm206xx_gyro_factory_device register failed!\n");
		goto exit;
	}

	res = icm206xx_gyro_create_attr(&(icm206xx_gyro_init_info.platform_diver_addr->driver));
	if (res) {
		GYRO_PR_ERR("icm206xx_g create attribute err = %d\n", res);
		goto exit_misc_device_register_failed;
	}

	ctl.is_use_common_factory = false;
	ctl.is_report_input_direct = false;
	ctl.batch = icm206xx_gyro_batch;
	ctl.flush = icm206xx_gyro_flush;
...
#ifdef CONFIG_TRAN_SYSTEM_DEVINFO
	app_get_gyro_sensor_name("icm206xx_gyro");
#endif
...
icm206xx_gyro_delete_attr(&(icm206xx_gyro_init_info.platform_diver_addr->driver));
exit_misc_device_register_failed:
gyro_factory_device_deregister(&icm206xx_gyro_factory_device);
exit:
...
static int icm206xx_gyro_i2c_remove(struct i2c_client *client)
{
	...
	if (res)
		GYRO_PR_ERR("icm206xx_gyro_delete_attr fail: %d\n", res);

	res = gyro_factory_device_deregister(&icm206xx_gyro_factory_device);
	if (res)
...
{
	strlcpy(info->type, ICM206XX_GYRO_DEV_NAME, sizeof(info->type));
	return 0;
}
