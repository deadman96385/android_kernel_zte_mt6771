...
int cts_plat_i2c_write(struct cts_platform_data *pdata, u8 i2c_addr, const void *src, size_t len, int retry, int delay)
{
	int ret = 0, retries = 0;
...
int cts_plat_i2c_read(struct cts_platform_data *pdata, u8 i2c_addr,
		      const u8 *wbuf, size_t wlen, void *rbuf, size_t rlen, int retry, int delay)
{
	...
	do {
		ret = i2c_transfer(pdata->i2c_client->adapter, msgs + ARRAY_SIZE(msgs) - num_msg, num_msg);
...
static void cts_plat_handle_irq(struct cts_platform_data *pdata)
{
	...
	if (ret && !tpd_fw_cdev.lcd_reset_processing) {
...
{
	struct cts_platform_data *pdata = container_of(work, struct cts_platform_data, ts_irq_work);
...
#ifdef CONFIG_CTS_OF
static int cts_plat_parse_dt(struct cts_platform_data *pdata, struct device_node *dev_node)
{
	int ret;
	...
	ret = of_property_read_u32(dev_node, CFG_CTS_OF_X_RESOLUTION_NAME, &pdata->res_x);
	if (ret) {
	...
	ret = of_property_read_u32(dev_node, CFG_CTS_OF_Y_RESOLUTION_NAME, &pdata->res_y);
	if (ret) {
...
int cts_init_platform_data(struct cts_platform_data *pdata, struct i2c_client *i2c_client)
{
	...
	input_mt_init_slots(input_dev, CFG_CTS_MAX_TOUCH_NUM, INPUT_MT_DIRECT);
...
int cts_plat_request_resource(struct cts_platform_data *pdata)
{
	...
	ret = gpio_request_one(pdata->int_gpio, GPIOF_IN, CFG_CTS_DEVICE_NAME "-int");
	if (ret) {
		cts_err("Request INT gpio (%d) failed %d", pdata->int_gpio, ret);
		goto err_out;
	}
#ifdef CFG_CTS_HAS_RESET_PIN
	ret = gpio_request_one(pdata->rst_gpio, GPIOF_OUT_INIT_HIGH, CFG_CTS_DEVICE_NAME "-rst");
	if (ret) {
...
int cts_plat_process_touch_msg(struct cts_platform_data *pdata, struct cts_device_touch_msg *msgs, int num)
{
	struct input_dev *input_dev = pdata->ts_input_dev;
	int i = 0;
	int contact = 0;
	static char finger_down[CFG_CTS_MAX_TOUCH_NUM] = {0};
	...
	if ((num == 0)  || (num > CFG_CTS_MAX_TOUCH_NUM)) {
		cts_err("Process touch num:%d", num);
		return 0;
	}
	memset(finger_current, 0, CFG_CTS_MAX_TOUCH_NUM);
	for (i = 0; i < num; i++) {
		u16 x, y;

		if (msgs[i].id > CFG_CTS_MAX_TOUCH_NUM) {
			cts_err("Process touch msgs[i].id : %d", msgs[i].id);
			return 0;
		}
		...
		case CTS_DEVICE_TOUCH_EVENT_STAY:
			...
			if (!finger_down[msgs[i].id]) {
				finger_down[msgs[i].id] = 1;
				cts_info("touch down id: %d, coord [%d:%d]", msgs[i].id, msgs[i].x, msgs[i].y);
			}
			break;
		...
		default:
			cts_warn("Process touch msg with unknwon event %u id %u", msgs[i].event, msgs[i].id);
			break;
		}
	}
	for (i = 0; i < CFG_CTS_MAX_TOUCH_NUM; i++) {
		if (finger_last[msgs[i].id] != 0 && finger_current[msgs[i].id] == 0) {
			input_mt_slot(input_dev, msgs[i].id);
			input_mt_report_slot_state(input_dev, MT_TOOL_FINGER, false);
			if (finger_down[msgs[i].id] == 1) {
				finger_down[msgs[i].id] = 0;
				cts_info("touch up id: %d, coord [%d:%d]", msgs[i].id, msgs[i].x, msgs[i].y);
			}
		}
		finger_last[msgs[i].id] = finger_current[msgs[i].id];
	}
...
int cts_plat_init_vkey_device(struct cts_platform_data *pdata)
{
	...
	for (i = 0; i < pdata->vkey_num; i++) {
		input_set_capability(pdata->ts_input_dev, EV_KEY, pdata->vkey_keycodes[i]);
	}
...
int cts_plat_process_vkey(struct cts_platform_data *pdata, u8 vkey_state)
{
	...
	for (i = 0; i < pdata->vkey_num; i++) {
		input_report_key(pdata->ts_input_dev, pdata->vkey_keycodes[i], vkey_state & BIT(i) ? 1 : 0);
	}
...
int cts_plat_init_gesture(struct cts_platform_data *pdata)
{
	...
	for (i = 0; i < pdata->gesture_num; i++) {
		input_set_capability(pdata->ts_input_dev, EV_KEY, pdata->gesture_keymap[i][1]);
	}
