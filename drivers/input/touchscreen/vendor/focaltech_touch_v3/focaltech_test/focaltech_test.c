...
*****************************************************************************/
extern int fts_save_failed_node(int failed_node);
extern char g_fts_save_file_path[];
extern char g_fts_save_file_name[];
...
struct test_funcs *test_func_list[] = {
	...
	&test_func_ft5822,
#elif (FTS_CHIP_TEST_TYPE == FT5422_TEST)
	&test_func_ft5422,
#elif (FTS_CHIP_TEST_TYPE == FT6426_TEST)
	&test_func_ft6426,
	...
	&test_func_ft8201,
#elif (FTS_CHIP_TYPE == _FT8006P)
	&test_func_ft8006p,
#elif (FTS_CHIP_TEST_TYPE == FT5452_TEST)
	&test_func_ft5452,
#elif (FTS_CHIP_TEST_TYPE == FT5472_TEST)
	&test_func_ft5472,
#elif (FTS_CHIP_TEST_TYPE == FT6216_TEST)
	&test_func_ft6216,
#elif (FTS_CHIP_TYPE == _FT7251)
	&test_func_ft7251,
#elif (FTS_CHIP_TEST_TYPE == FT8613_TEST)
	&test_func_ft8613s,
#endif
};
...
static int fts_test_save_test_data(char *file_name, char *data_buf, int len)
{
	...
	if (strnstr(file_name, ".csv", strlen(file_name))) {
		snprintf(filepath, sizeof(filepath), "%s%s.csv", g_fts_save_file_path, g_fts_save_file_name);
	} else if (strnstr(file_name, ".txt", strlen(file_name))) {
		snprintf(filepath, sizeof(filepath), "%s%s.txt", g_fts_save_file_path, g_fts_save_file_name);
	} else {
		snprintf(filepath, sizeof(filepath), "%s%s", FTS_SAVE_FILE_PATH, file_name);
	}
	if (pfile == NULL) {
	...
	for (i = 0; i < td->item_count; i++) {
		...
		for (j = 1; j <= data_count; j++) {
			line2_length += snprintf(line2_buffer + line2_length,
						 ...
						 "%s, %d, %d, %d, %d, %d, ",
						 info->name, info->code, tx, rx, start_line, j);
			start_line += tx;
			csv_item_count++;
		}
	}
...
void fts_test_save_data(char *name, int code, int *data, int datacnt, bool mc_sc, bool key, bool result)
{
	...
	strlcpy(info->name, name, TEST_ITEM_NAME_MAX);
	info->code = code;
	info->mc_sc = mc_sc;
