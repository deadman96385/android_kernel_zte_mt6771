...
static ssize_t fts_debug_write(struct file *filp, const char __user *buff, size_t count, loff_t *ppos)
{
	...
	if ((buflen <= 1) || (buflen > PAGE_SIZE)) {
		FTS_ERROR("apk proc write count(%d>%d) fail", buflen, (int)PAGE_SIZE);
		return -EINVAL;
	}
	...
	case PROC_READ_DATA:
		writelen = buflen - 1 >= FTX_MAX_COMMMAND_LENGTH ? FTX_MAX_COMMMAND_LENGTH : buflen - 1;
	...
	case PROC_SET_SLAVE_ADDR:
		...
		if (writebuf[1] != ts_data->client->addr) {
			...
			FTS_INFO("Change i2c addr 0x%x to 0x%x", ts_data->client->addr << 1, writebuf[1] << 1);
		}
#endif
		break;
...
static ssize_t fts_log_level_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	int count = 0;
	struct input_dev *input_dev = fts_data->input_dev;
...
static ssize_t fts_log_level_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	int value = 0;
	int ret = 0;
	struct input_dev *input_dev = fts_data->input_dev;
...
static DEVICE_ATTR(fts_fw_version, 0664, fts_tpfwver_show, fts_tpfwver_store);
...
*       cat rw_reg
*/
static DEVICE_ATTR(fts_rw_reg, 0664, fts_tprwreg_show, fts_tprwreg_store);
/*  upgrade from fw bin file   example:echo "*.bin" > fts_upgrade_bin */
static DEVICE_ATTR(fts_upgrade_bin, 0664, fts_fwupgradebin_show, fts_fwupgradebin_store);
static DEVICE_ATTR(fts_force_upgrade, 0664, fts_fwforceupg_show, fts_fwforceupg_store);
static DEVICE_ATTR(fts_driver_info, 0664, fts_driverinfo_show, fts_driverinfo_store);
static DEVICE_ATTR(fts_dump_reg, 0664, fts_dumpreg_show, fts_dumpreg_store);
static DEVICE_ATTR(fts_hw_reset, 0664, fts_hw_reset_show, fts_hw_reset_store);
static DEVICE_ATTR(fts_irq, 0664, fts_irq_show, fts_irq_store);
static DEVICE_ATTR(fts_boot_mode, 0664, fts_bootmode_show, fts_bootmode_store);
static DEVICE_ATTR(fts_touch_point, 0664, fts_tpbuf_show, fts_tpbuf_store);
static DEVICE_ATTR(fts_log_level, 0664, fts_log_level_show, fts_log_level_store);
