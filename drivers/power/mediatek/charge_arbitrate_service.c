...
#define POWER_SUPPLY_PROP_CHARGE_VOLTAGE_MAX_ZTE POWER_SUPPLY_PROP_CONSTANT_CHARGE_VOLTAGE_MAX
#endif

#ifndef POWER_SUPPLY_PROP_CHARGE_CURRENT_MAX_ZTE
#define POWER_SUPPLY_PROP_CHARGE_CURRENT_MAX_ZTE POWER_SUPPLY_PROP_CONSTANT_CHARGE_CURRENT_MAX
#endif

#ifndef POWER_SUPPLY_PROP_RECHARGE_SOC_ZTE
#define POWER_SUPPLY_PROP_RECHARGE_SOC_ZTE POWER_SUPPLY_PROP_SCOPE
#endif
...
#define POWER_SUPPLY_PROP_RECHARGE_VOLTAGE_ZTE POWER_SUPPLY_PROP_CALIBRATE
#endif

/*
#ifndef POWER_SUPPLY_PROP_CAPACITY_RAW_ZTE
#define POWER_SUPPLY_PROP_CAPACITY_RAW_ZTE POWER_SUPPLY_PROP_CAPACITY_RAW_ZTE
#endif
*/

#ifndef POWER_SUPPLY_PROP_INPUT_CURRENT_LIMIT_ZTE
#define POWER_SUPPLY_PROP_INPUT_CURRENT_LIMIT_ZTE POWER_SUPPLY_PROP_INPUT_CURRENT_LIMIT
#endif
...
struct dts_config_prop {
	...
	const char	*fcv_phy;
	const char	*fcc_phy;
	const char	*topoff_phy;
	const char	*icl_phy;
	const char	*recharge_soc_phy;
	const char	*recharge_voltage_phy;
	const char	*voltage_cc_cv_phy;
...
static int charge_arbitrate_update_chip_status(struct charge_arbitrate_info *cas_info)
{
	...
	if (cas_info->run_prop.run_fcc != run_prop_prev.run_fcc) {
		power_supply_set_prop_by_name(cas_info->config_prop.fcc_phy,
	...
	if (cas_info->run_prop.run_fcv != run_prop_prev.run_fcv) {
		power_supply_set_prop_by_name(cas_info->config_prop.fcv_phy,
	...
	if (cas_info->run_prop.run_fcv != run_prop_prev.run_fcv) {
		power_supply_set_prop_by_name(cas_info->config_prop.voltage_cc_cv_phy,
	...
	if (cas_info->run_prop.run_rech_soc != run_prop_prev.run_rech_soc) {
		power_supply_set_prop_by_name(cas_info->config_prop.recharge_soc_phy,
	...
	if (cas_info->run_prop.run_rech_voltage != run_prop_prev.run_rech_voltage) {
		power_supply_set_prop_by_name(cas_info->config_prop.recharge_voltage_phy,
	...
	if (cas_info->run_prop.topoff_current_percent != run_prop_prev.topoff_current_percent) {
		...
		power_supply_set_prop_by_name(cas_info->config_prop.topoff_phy,
...
static int charge_arbitrate_parse_dt(struct charge_arbitrate_info *cas_info)
{
	...
	OF_READ_PROPERTY_STRINGS(cas_info->config_prop.fcv_phy,
			"fcv-phy", retval, "battery");

	OF_READ_PROPERTY_STRINGS(cas_info->config_prop.fcc_phy,
			"fcc-phy", retval, "battery");

	OF_READ_PROPERTY_STRINGS(cas_info->config_prop.topoff_phy,
			"topoff-phy", retval, "battery");

	OF_READ_PROPERTY_STRINGS(cas_info->config_prop.recharge_soc_phy,
			"recharge-soc-phy", retval, "battery");

	OF_READ_PROPERTY_STRINGS(cas_info->config_prop.recharge_voltage_phy,
			"recharge-voltage-phy", retval, "battery");

	OF_READ_PROPERTY_STRINGS(cas_info->config_prop.voltage_cc_cv_phy,
			"voltage-cc-cv-phy", retval, "bms");

	OF_READ_PROPERTY_STRINGS(cas_info->config_prop.icl_phy,
			"icl-phy", retval, "battery");
...
static int charge_arbitrate_set_input_suspend(struct charge_arbitrate_info *cas_info, bool flag)
{
	...
	rc = power_supply_set_prop_by_name(cas_info->config_prop.icl_phy,
			POWER_SUPPLY_PROP_INPUT_CURRENT_LIMIT_ZTE, flag);
	if (rc < 0) {
		pr_alarm("Failed to set INPUT_CURRENT_LIMIT property rc=%d\n", rc);
		goto failed_loop;
	}
...
static int charge_arbitrate_r1_p1_status_chargefull(struct charge_arbitrate_info *cas_info)
{
	...
	case POWER_SUPPLY_STATUS_CHARGING:
		if ((cas_info->run_prop.run_rech_soc != 0)/*check soc_now less than recharge soc*/
			&& ((capacity_raw.intval * 100 / FULL_RAW_SOC) < cas_info->run_prop.run_rech_soc)) {
			cas_info->force_charger_full = false;
			charge_arbitrate_set_input_suspend(cas_info, false);
			pr_alarm("<<<---Exit force charge full; soc now: %d, recharge soc: %d\n",
								(capacity_raw.intval * 100 / FULL_RAW_SOC),
								cas_info->run_prop.run_rech_soc);
...
static int charge_arbitrate_probe(struct platform_device *pdev)
{
	...
	queue_delayed_work(cas_info->cas_probe_wq, &cas_info->cas_probe_work, msecs_to_jiffies(100));
