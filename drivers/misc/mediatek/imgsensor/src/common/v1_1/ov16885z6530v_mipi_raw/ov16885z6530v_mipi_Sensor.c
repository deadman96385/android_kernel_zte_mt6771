...
#include "ov16885z6530v_mipi_Sensor.h"
...
static SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[5] = {
	...
	{4608, 3456, 0, 0, 4608, 3456, 2304, 1728,  0, 0, 2304, 1728,  0, 0, 2304, 1728}, /* hight speed video*/
...
static void set_shutter(kal_uint16 shutter)
{
	...
	/* Update Shutter */
	LOG_INF("mtk_test: remosaic flag = %d\n", remosaic_flag);
	remosaic_shutter = shutter * 4;

	if (remosaic_flag == 0) {
		write_cmos_sensor(0x3502, (shutter << 4) & 0xF0);
		write_cmos_sensor(0x3501, (shutter >> 4) & 0xFF);
		write_cmos_sensor(0x3500, (shutter >> 12) & 0x0F);
		gain_compensat_flag = 0;
	} else if (remosaic_shutter <= (imgsensor.frame_length-12) && remosaic_flag == 1) {
		LOG_INF("mtk_test: set remosaic_shutter %d ", remosaic_shutter);
		write_cmos_sensor(0x3502, (remosaic_shutter << 4) & 0xF0);
		write_cmos_sensor(0x3501, (remosaic_shutter >> 4) & 0xFF);
		write_cmos_sensor(0x3500, (remosaic_shutter >> 12) & 0x0F);
		gain_compensat_flag = 0;
	} else {
		LOG_INF("mtk_test: set shutter as frame length %d ", remosaic_shutter);
		remosaic_shutter = imgsensor.frame_length-12;
		write_cmos_sensor(0x3502, (remosaic_shutter << 4) & 0xF0);
		write_cmos_sensor(0x3501, (remosaic_shutter >> 4) & 0xFF);
		write_cmos_sensor(0x3500, (remosaic_shutter >> 12) & 0x0F);
		gain_compensat_flag = 1;
	}
...
write_cmos_sensor(0x3501, 0x8e);
write_cmos_sensor(0x3502, 0xd0);
...
write_cmos_sensor(0x481f, 0x2b);
write_cmos_sensor(0x4821, 0x65);
...
{
	LOG_INF("streaming_enable123(0=Sw Standby,1=streaming): %d\n", enable);
	if (enable)
...
static void preview_setting(void)
{
	...
	write_cmos_sensor(0x3813, 0x13);
	write_cmos_sensor(0x3815, 0x22);
	write_cmos_sensor(0x3820, 0x01);
	write_cmos_sensor(0x3834, 0xf5);
	...
	write_cmos_sensor(0x4202, 0x01);
	write_cmos_sensor(0x4500, 0x25);
	...
	write_cmos_sensor(0x5084, 0x09);
	write_cmos_sensor(0x5085, 0x30);
	write_cmos_sensor(0x5086, 0x06);
	write_cmos_sensor(0x5087, 0xe6);
	write_cmos_sensor(0x5000, 0x89);
	write_cmos_sensor(0x5001, 0x42);
	write_cmos_sensor(0x5004, 0x81);
	write_cmos_sensor(0x5184, 0x63);
	write_cmos_sensor(0x5185, 0x17);
	write_cmos_sensor(0x5192, 0x09);
	write_cmos_sensor(0x5193, 0x2f);
} /*      preview_setting  */
...
UINT32 OV16885_MIPI_RAW_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
