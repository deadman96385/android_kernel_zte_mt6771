...
#define AWB_FLAG1_ADDRESS  0x0A25
...
static kal_uint8 zte_s5k4h7_127_checksum(unsigned char *buf,
	unsigned int first, unsigned int last, unsigned int position)
{
	...
	for (i = first; i <= last; i++) {
		buf[j] = (kal_uint8) read_cmos_sensor_8(i)&0xff;
		sum += buf[j];
		j++;
	}
	buf[j] = (kal_uint8) read_cmos_sensor_8(position)&0xff;
	/*LOG_INF("buf[%d] =%d,sum=%d\n",j,buf[j],sum);*/
	if ((sum % 256) == buf[j]) {
		LOG_INF(" Checksum good, j:%d, sum:%d buf: %d", j,
		sum, buf[j]);
		ret = 1;
	}
	return ret;
...
static bool zte_s5k4h7_127_read_info(void)
{
	kal_uint16 flag1, flag2, flag3;
	...
	flag = s5k4H7_start_read_otp(INFO_AWB_PAGE);
	if (!flag) {
		LOG_INF("invalid otp select page");
	}
	flag1 = read_cmos_sensor_8(MODULE_ID_FLAG1_ADDRESS);
	flag2 = read_cmos_sensor_8(MODULE_ID_FLAG2_ADDRESS);
	flag3 = read_cmos_sensor_8(MODULE_ID_FLAG3_ADDRESS);
	/* LOG_INF("flag=%d, flag1=0x%x, flag2=0x%x, flag3=0x%x\n", flag, flag1, flag2, flag3); */
	if (flag1 == 1)
		start_address = MODULE_ID_FLAG1_ADDRESS + 1;
	else if (flag2 == 1)
		start_address = MODULE_ID_FLAG2_ADDRESS + 1;
	else if (flag3 == 1)
		start_address = MODULE_ID_FLAG3_ADDRESS + 1;
	else {
		LOG_INF("invalid otp data");
		return false;
	}
	buf = kmalloc(sizeof(kal_uint8)*8, GFP_KERNEL);
	checksum = zte_s5k4h7_127_checksum(buf, start_address, start_address + 7, start_address + 8);
	if (checksum == 1) {
		otp_data_info.module_integrator_id = buf[start_address];
		otp_data_info.lens_id = buf[start_address+1];
		otp_data_info.production_year = buf[start_address+4];
		otp_data_info.production_month = buf[start_address+5];
		otp_data_info.production_day = buf[start_address+6];
		/* LOG_INF("otp info data module_integrator_id = 0x%x\n", otp_data_info.module_integrator_id);*/
		ret = true;
	} else {
...
static void apply_zte_4h7_otp_awb(void)
{
	uint16_t R_gain, G_gain, B_gain, Base_gain;
	uint16_t rg, bg, rg_ratio_typical, bg_ratio_typical;

	rg = otp_data_info.r_cur;
	bg = otp_data_info.b_cur;

	rg_ratio_typical = otp_data_info.r_golden_cur;
	bg_ratio_typical = otp_data_info.b_golden_cur;
	...
	if (R_gain > 0x100) {
		write_cmos_sensor_8(0x0210, ((R_gain >> 8) & 0xFF));
		write_cmos_sensor_8(0x0211, R_gain & 0xFF);
	}
	if (G_gain > 0x100) {
		write_cmos_sensor_8(0x020E, (G_gain >> 8) & 0xFF);
		write_cmos_sensor_8(0x020F, G_gain & 0xFF);

		write_cmos_sensor_8(0x0214, (G_gain >> 8) & 0xFF);
		write_cmos_sensor_8(0x0215, G_gain & 0xFF);
	}
	if (B_gain > 0x100) {
		write_cmos_sensor_8(0x0212, (B_gain >> 8) & 0xFF);
...
static bool zte_s5k4h7_127_awb(void)
{
	...
	bool flag = false;
	bool ret = false;
	unsigned int i = 0, j = 2, sum = 0;

	flag = s5k4H7_start_read_otp(INFO_AWB_PAGE);
	if (!flag) {
		LOG_INF("invalid otp select page");
	}
	flag1 = read_cmos_sensor_8(AWB_FLAG1_ADDRESS);
	flag2 = read_cmos_sensor_8(AWB_FLAG2_ADDRESS);
	flag3 = read_cmos_sensor_8(AWB_FLAG3_ADDRESS);
	/* LOG_INF("flag = %d, flag1 = 0x%x, flag2 = 0x%x, flag3 = 0x%x\n",
	 flag, flag1, flag2, flag3); */
	if (flag1 == 1) {
		start_address = AWB_FLAG1_ADDRESS + 1;
		otp_data_info.awb_group = 1;
	} else if (flag2 == 1) {
		start_address = AWB_FLAG2_ADDRESS + 1;
		otp_data_info.awb_group = 2;
	} else if (flag3 == 1) {
		start_address = AWB_FLAG3_ADDRESS + 1;
		otp_data_info.awb_group = 3;
	} else {
		LOG_INF("invalid otp data");
		return false;
	}
	buf = kmalloc(sizeof(kal_uint8)*12, GFP_KERNEL);
	if (otp_data_info.awb_group == 3) {
		buf[0] = (kal_uint8) read_cmos_sensor_8(start_address)&0xff;
		sum += buf[0];
		buf[1] = (kal_uint8) read_cmos_sensor_8(start_address + 1)&0xff;
		sum += buf[1];
		/* select 22 page */
		flag = s5k4H7_start_read_otp(INFO_AWB_GROUP3_PAGE);
		if (!flag) {
			LOG_INF("invalid  select awb page 3");
			return false;
		}
		for (i = MODULE_ID_FLAG1_ADDRESS; i <= MODULE_ID_FLAG1_ADDRESS+9; i++) {
			buf[j] = (kal_uint8) read_cmos_sensor_8(i)&0xff;
			sum += buf[j];
			j++;
		}
		buf[j] = (kal_uint8) read_cmos_sensor_8(MODULE_ID_FLAG1_ADDRESS+10)&0xff;
		/* LOG_INF("group 3 buf[%d] =%d, sum=%d\n", j, buf[j], sum); */
		if ((sum % 256) == buf[j]) {
			LOG_INF(" Checksum good, j:%d, sum:%d buf: %d", j,
			sum, buf[j]);
			checksum = 1;
		...
		checksum = zte_s5k4h7_127_checksum(buf, start_address, start_address + 11, start_address + 12);
	}
	if (checksum == 1) {
		otp_data_info.r_cur = buf[0] << 8 | buf[1];
		otp_data_info.b_cur = buf[2] << 8 | buf[3];
		/* otp_data_info.gbgr_cur = buf[4] << 8 | buf[5]; */
		otp_data_info.r_golden_cur = buf[6] << 8 | buf[7];
		otp_data_info.b_golden_cur = buf[8] << 8 | buf[9];
		/* otp_data_info.gbgr_golden_cur = buf[10] << 8 | buf[11]; */
...
static unsigned int sum_awb_flag_lsc(unsigned int sum_start, unsigned int sum_end, unsigned char *pdata)
{
	int i = 0;
	unsigned int start;
	...
	if (flag1 != 0) {
		...
		for (page_start = 1, page_end = 6; page_start <= page_end; page_start++) {
			...
			if (page_start == 6) {
				...
				/* LOG_INF("flag =%d\n", flag); */
				if (flag) {
					sum_slc += sum_awb_flag_lsc(0x0A04, 0x0A2A, data_p[page_start-1]);
					continue;
				}
			}
		...
		if (flag2 != 0) {
			...
			for (page_start = 6, page_end = 12; page_start <= page_end; page_start++) {
				...
				if (page_start == 12) {
					flag = s5k4H7_start_read_otp(page_start);
					if (flag) {
					sum_slc += sum_awb_flag_lsc(0x0A04, 0x0A12, data_p[page_start-1]);
					continue;
					}
				}
				if (page_start == 6) {
...
static void normal_video_setting(kal_uint16 currefps)
{
	...
	LOG_INF("s5k normal_video_setting");
	/* Reset for operation */
	write_cmos_sensor_8(0X0100, 0X00);
	write_cmos_sensor_8(0X0136, 0X18);
	write_cmos_sensor_8(0X0137, 0X00);
