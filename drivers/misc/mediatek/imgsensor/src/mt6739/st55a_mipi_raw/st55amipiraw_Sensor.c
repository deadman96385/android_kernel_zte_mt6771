...
#include "st55amipiraw_Sensor.h"

/****************************Modify following Strings for debug****************************/
#define PFX "st55a_camera_sensor"
#define LOG_1 LOG_INF("st55a,MIPI 2LANE\n")
/****************************   Modify end    *******************************************/
...
static imgsensor_struct imgsensor = {
	...
	.autoflicker_en = KAL_FALSE,  /*KAL_FALSE for disable auto flicker*/
...
static void set_max_framerate(UINT16 framerate, kal_bool min_framelength_en)
{
	...
	LOG_INF("framerate = %d, min framelength is %d\n", framerate, min_framelength_en);
...
static void preview_setting(void)
{
	...
	write_cmos_sensor(0x0307, 0xD6);
	write_cmos_sensor(0x3C1F, 0x00); /*PLLS*/

	write_cmos_sensor(0x0820, 0x03); /* requested link bit rate mbps*/
	...
	/* Size Setting*/
	write_cmos_sensor(0x0340, 0x07); /* frame_length_lines : def. 990d*/
...
static void capture_setting(kal_uint16 currefps)
{
	...
	if (currefps == 150) {
		...
		write_cmos_sensor(0x0820, 0x01); /* requested link bit rate mbps*/
		...
		/* Size Setting*/
		write_cmos_sensor(0x0340, 0x07); /* frame_length_lines*/
		...
		write_cmos_sensor(0x0202, 0x02);  /* coarse integration*/
		write_cmos_sensor(0x0203, 0x00);
		write_cmos_sensor(0x0200, 0x04);  /* fine integration*/
		write_cmos_sensor(0x0201, 0x98);
...
static void hs_video_setting(void)
{
	...
	/* Size Setting*/
	write_cmos_sensor(0x0340, 0x01); /* frame_length_lines*/
...
static kal_uint32 open(void)
{
	...
	if ((imgsensor_info.sensor_id != sensor_id) && (sensor_id != 0x55A0))
	return ERROR_SENSOR_CONNECT_FAIL;
...
UINT32 ST55A_MIPI_RAW_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
