...
#ifdef TLSC_RESET_SEQUENCE
void tlsc6x_tpd_set_reset_gpio(int value)
{
	if (value == 1) {
#ifdef CONFIG_TS_TLSC6X_MTK_INTERFACE
...
#define PROC_OUT_BUF_SIZE 256
unsigned char proc_out_len;
unsigned char proc_out_buf[PROC_OUT_BUF_SIZE];
...
static ssize_t tlsc6x_proc_read(struct file *filp, char __user *page, size_t len, loff_t *pos)
{
	...
	case 3:
		break;
	case 4:
		if (len > PROC_OUT_BUF_SIZE) {
			tlsc_err("%s len(%d) is invalid\n", __func__, len);
			return -EINVAL;
		}
...
static int tlsc6x_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	/* add i2c retry */
	while (reset_count++ < 3) {
		/* just check iic connection, any register address is ok */
		if (tlsc6x_i2c_read(this_client, buf, 1, buf, 1) >= 0) {
			tlsc_info("i2c retry time %d\n", reset_count);
			break;
		}
		tlsc6x_tpd_reset();
		msleep(30);
