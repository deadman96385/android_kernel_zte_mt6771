...
#define AW8898_CFG_NAME_MAX   64

static char aw8898_cfg_name[][AW8898_CFG_NAME_MAX] = {
	{"aw8898_music_spk_cfg.bin"},
	{"aw8898_voice_rcv_cfg.bin"},
	{"aw8898_voice_spk_cfg.bin"},
};
...
static void aw8898_cfg_loaded(const struct firmware *cont, void *context)
{
	...
	if (!cont) {
		pr_err("%s: failed to read %s\n", __func__, aw8898_cfg_name[aw8898->spk_rcv_mode]);
	...
	aw8898->init = aw8898->spk_rcv_mode + AW8898_MAX_MODE;
...
static int aw8898_load_cfg(struct aw8898 *aw8898)
{
	...
	return request_firmware_nowait(THIS_MODULE, FW_ACTION_HOTPLUG,
			aw8898_cfg_name[aw8898->spk_rcv_mode], aw8898->dev, GFP_KERNEL,
...
static void aw8898_smartpa_cfg(struct aw8898 *aw8898, bool flag)
{
	...
	if (flag == true) {
		/* The firmware must be reloaded when the smartPA mode is switched */
		if (aw8898->init != (aw8898->spk_rcv_mode + AW8898_MAX_MODE)) {
 ...
 *
 ******************************************************/
static const char *const music_spk_function[] = { "Off", "On" };
static const char *const voice_rcv_function[] = { "Off", "On" };
static const char *const voice_spk_function[] = { "Off", "On" };
static const char *const speaker_function[]   = { "Off", "On" };
static int aw8898_music_spk_control = 0;
static int aw8898_voice_rcv_control = 0;
static int aw8898_voice_spk_control = 0;
...
static int aw8898_music_spk_get(struct snd_kcontrol *kcontrol,
		struct snd_ctl_elem_value *ucontrol)
{
	pr_debug("%s: aw8898_music_spk_control=%d\n", __func__, aw8898_music_spk_control);
	ucontrol->value.integer.value[0] = aw8898_music_spk_control;
	return 0;
}

static int aw8898_music_spk_set(struct snd_kcontrol *kcontrol,
		struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_soc_kcontrol_codec(kcontrol);
	struct aw8898 *aw8898 = snd_soc_codec_get_drvdata(codec);
	...
	if (ucontrol->value.integer.value[0] == aw8898_music_spk_control)
		return 1;

	aw8898_music_spk_control = ucontrol->value.integer.value[0];

	aw8898->spk_rcv_mode = AW8898_MUSIC_SPEAKER_MODE;
...
static int aw8898_voice_rcv_get(struct snd_kcontrol *kcontrol,
		struct snd_ctl_elem_value *ucontrol)
{
	pr_debug("%s: aw8898_voice_rcv_control=%d\n", __func__, aw8898_voice_rcv_control);
	ucontrol->value.integer.value[0] = aw8898_voice_rcv_control;
	return 0;
}
static int aw8898_voice_rcv_set(struct snd_kcontrol *kcontrol,
		struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_soc_kcontrol_codec(kcontrol);
	struct aw8898 *aw8898 = snd_soc_codec_get_drvdata(codec);
	...
	pr_debug("%s: ucontrol->value.integer.value[0]=%ld\n ",
			__func__, ucontrol->value.integer.value[0]);
	if (ucontrol->value.integer.value[0] == aw8898_voice_rcv_control)
		return 1;

	aw8898_voice_rcv_control = ucontrol->value.integer.value[0];

	aw8898->spk_rcv_mode = AW8898_VOICE_RECEIVER_MODE;
...
static int aw8898_voice_spk_get(struct snd_kcontrol *kcontrol,
		struct snd_ctl_elem_value *ucontrol)
{
	pr_info("%s: aw8898_voice_spk_control=%d\n", __func__, aw8898_voice_spk_control);
	ucontrol->value.integer.value[0] = aw8898_voice_spk_control;
	return 0;
}

static int aw8898_voice_spk_set(struct snd_kcontrol *kcontrol,
		struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_soc_kcontrol_codec(kcontrol);
	struct aw8898 *aw8898 = snd_soc_codec_get_drvdata(codec);
	...
	pr_debug("%s: ucontrol->value.integer.value[0]=%ld\n ",
			__func__, ucontrol->value.integer.value[0]);
	if (ucontrol->value.integer.value[0] == aw8898_voice_spk_control)
		return 1;

	aw8898_voice_spk_control = ucontrol->value.integer.value[0];

	aw8898->spk_rcv_mode = AW8898_VOICE_SPEAKER_MODE;
...
static int aw8898_speaker_get(struct snd_kcontrol *kcontrol,
		struct snd_ctl_elem_value *ucontrol)
{
	pr_info("%s: unsupported operation\n", __func__);
	return 0;
}

static int aw8898_speaker_set(struct snd_kcontrol *kcontrol,
		struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_soc_kcontrol_codec(kcontrol);
	struct aw8898 *aw8898 = snd_soc_codec_get_drvdata(codec);
	...
	aw8898_smartpa_cfg(aw8898, false);
	if (aw8898_music_spk_control == 1) {
		aw8898_music_spk_control = 0;
	}

	if (aw8898_voice_spk_control == 1) {
		aw8898_voice_spk_control = 0;
	}

	if (aw8898_voice_rcv_control == 1) {
		aw8898_voice_rcv_control = 0;
	}
...
static const struct soc_enum aw8898_snd_enum[] = {
	SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(music_spk_function), music_spk_function),
	SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(voice_rcv_function), voice_rcv_function),
	SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(voice_spk_function), voice_spk_function),
	SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(speaker_function), speaker_function),
};

static struct snd_kcontrol_new aw8898_controls[] = {
	SOC_ENUM_EXT("aw8898_music_spk_switch", aw8898_snd_enum[0],
			aw8898_music_spk_get, aw8898_music_spk_set),
	SOC_ENUM_EXT("aw8898_voice_rcv_switch", aw8898_snd_enum[1],
			aw8898_voice_rcv_get, aw8898_voice_rcv_set),
	SOC_ENUM_EXT("aw8898_voice_spk_switch", aw8898_snd_enum[2],
			aw8898_voice_spk_get, aw8898_voice_spk_set),
	SOC_ENUM_EXT("aw8898_speaker_switch", aw8898_snd_enum[3],
			aw8898_speaker_get, aw8898_speaker_set),
};
		...
		char *buf)
{
	...
	if (aw8898->spk_rcv_mode == AW8898_MUSIC_SPEAKER_MODE) {
		len += snprintf(buf+len, PAGE_SIZE-len,
				"aw8898 spk_rcv: %d, music speaker mode\n", aw8898->spk_rcv_mode);
	} else if (aw8898->spk_rcv_mode == AW8898_VOICE_RECEIVER_MODE) {
		len += snprintf(buf+len, PAGE_SIZE-len,
				"aw8898 spk_rcv: %d, voice receiver mode\n", aw8898->spk_rcv_mode);
	} else if (aw8898->spk_rcv_mode == AW8898_VOICE_SPEAKER_MODE) {
		len += snprintf(buf+len, PAGE_SIZE-len,
				"aw8898 spk_rcv: %d, voice speaker mode\n", aw8898->spk_rcv_mode);
	} else {
