...
static int tpd_enable_wakegesture(struct tpd_classdev_t *cdev, int enable)
{
	...
	fts_gesture_mode = enable;

	return enable;
}

#if FTS_COVER_EN
extern struct fts_mode_flag g_fts_mode_flag;
extern int fts_enter_cover_mode(struct i2c_client *client, int mode);
static int tpd_get_smart_cover(struct tpd_classdev_t *cdev)
{
	cdev->b_smart_cover_enable = g_fts_mode_flag.fts_cover_mode_flag;
	return 0;
}
...
static int fts_set_tp_suspend(struct tpd_classdev_t *cdev, int enable)
{
	...
	if (enable) {
		...
		if (fts_data->fb_tpd_suspend_flag) {
			...
			ret = queue_work(fts_data->ts_workqueue, &fts_data->fb_tpd_resume_work);
			if (!ret) {
...
static int tpd_register_fw_class(struct fts_ts_data *data)
{
	...
	tpd_fw_cdev.tp_hw_reset = fts_hw_reset;
	return 0;
}
	...
	for (i = 0; i < data->touch_point; i++) {
		...
		if (EVENT_DOWN(events[i].flag)) {
...
#ifdef FTS_SMART_AREA
			FTS_DBG("report_data %d %d %d %d\n", events[i].x, events[i].y,
					events[i].area, events[i].p);
			touch_major = events[i].area * 2;
			touch_minor = events[i].p * 2;
#else
			if (events[i].area <= 0) {
				events[i].area = 0x09;
			}
...
#ifdef FTS_SMART_AREA
			input_report_abs(data->input_dev, ABS_MT_TOUCH_MAJOR, touch_major);
			input_report_abs(data->input_dev, ABS_MT_TOUCH_MINOR, touch_minor);
#else
			...
			if (!finger_down[i]) {
				...
				log_y = events[i].y;
				finger_down[i] = 1;
				FTS_INFO("touch down id: %d, coord [%d:%d]\n", events[i].id, log_x, log_y);
			}
			...
			if (finger_down[i] == 1) {
				...
				log_y = events[i].y;
				finger_down[i] = 0;
				FTS_INFO("touch up id: %d, coord [%d:%d]\n", events[i].id, log_x, log_y);
			}
		}
	}
...
static int fb_notifier_callback(struct notifier_block *self, unsigned long event, void *data)
{
	...
	if (evdata && evdata->data && event == FB_EARLY_EVENT_BLANK && fts_data && fts_data->client) {
		blank = evdata->data;
