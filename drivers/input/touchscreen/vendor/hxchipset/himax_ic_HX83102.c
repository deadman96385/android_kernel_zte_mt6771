...
static void hx83102ab_set_SMWP_enable(uint8_t SMWP_enable, bool suspended)
{
	...
	do {
		...
		tmp_data[1] != back_data[1]  || tmp_data[0] != back_data[0]) && retry_cnt < HIMAX_REG_RETRY_TIMES);
...
{
	uint8_t cnt = 0;
	uint8_t  data_fw_stop[FOUR_BYTE_ADDR_SZ];
	uint8_t tmp_addr[FOUR_BYTE_ADDR_SZ];
	uint8_t tmp_data[FOUR_BYTE_DATA_SZ];
	uint8_t addr_ctrl_fw_isr[FOUR_BYTE_DATA_SZ];
	uint8_t addr_cs_central_state[FOUR_BYTE_DATA_SZ];

	himax_in_parse_assign_cmd(fw_data_fw_stop, data_fw_stop, sizeof(data_fw_stop));
	himax_in_parse_assign_cmd(fw_addr_ctrl_fw, addr_ctrl_fw_isr, sizeof(addr_ctrl_fw_isr));
	himax_in_parse_assign_cmd(ic_adr_cs_central_state, addr_cs_central_state, sizeof(addr_cs_central_state));

	do {
		if (cnt == 0 || (tmp_data[0] != 0xA5 && tmp_data[0] != 0x00 && tmp_data[0] != 0x87)) {
			hx83102_flash_write_burst(addr_ctrl_fw_isr, data_fw_stop);
		}
		msleep(20);

		/* check fw status */
		hx83102_register_read(addr_cs_central_state, FOUR_BYTE_ADDR_SZ, tmp_data);
		...
		hx83102_register_read(addr_ctrl_fw_isr, FOUR_BYTE_ADDR_SZ, tmp_data);
...
{
	I("%s:Entering!\n", __func__);
	himax_in_parse_assign_cmd(hx83102ab_fw_addr_sorting_mode_en,
		pfw_op->addr_sorting_mode_en, sizeof(pfw_op->addr_sorting_mode_en));
	himax_in_parse_assign_cmd(hx83102ab_fw_addr_selftest_addr_en,
		pfw_op->addr_selftest_addr_en, sizeof(pfw_op->addr_selftest_addr_en));
#ifdef HX_ZERO_FLASH
	himax_in_parse_assign_cmd(hx83102ab_data_adc_cfg_1,
...
static void himax_hx83102ab_func_re_init(void)
{
	...
	g_core_fp.fp_sense_off = hx83102_sense_off;
...
{
	I("%s:Entering!\n", __func__);
	himax_in_parse_assign_cmd(hx83102d_fw_addr_raw_out_sel,
		pfw_op->addr_raw_out_sel, sizeof(pfw_op->addr_raw_out_sel));
#if defined(HX_ZERO_FLASH)
	himax_in_parse_assign_cmd(hx83102d_zf_data_sram_start_addr,
...
static bool hx83102_chip_detect(void)
{
	...
	for (i = 0; i < 5; i++) {
		...
		if ((tmp_data[3] == 0x83) && (tmp_data[2] == 0x10) && ((tmp_data[1] == 0x2a) ||
			(tmp_data[1] == 0x2b) || (tmp_data[1] == 0x2d))) {
			if (tmp_data[1] == 0x2a) {
