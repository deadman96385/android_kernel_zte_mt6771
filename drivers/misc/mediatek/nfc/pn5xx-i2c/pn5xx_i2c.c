...
#define SIG_NFC 44
#define PN5XX_DRIVER_NAME         "pn544"
...
static long pn5xx_dev_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case PN5XX_SET_PWR:
	{
		if (arg == 2) {
		    ...
		    */
			...
			/* power on */
			NFC_DBG_MSG("power on,ven_gpio:%d\n", pn5xx_dev->firm_gpio);
			...
			if (clk_run == false) {
			   msleep(20);
			   ret = clk_set_parent(bb_clk2, source);
		       if (ret) {
					pr_err("%s: prepare source_clk failed ret:%d\n", __func__, ret);
			   }
			   ret = clk_prepare_enable(bb_clk2);
		       if (ret) {
					pr_err("%s: prepare bb_clk2 failed ret:%d\n", __func__, ret);
			   }
			   ret = clk_prepare_enable(enable);
		       if (ret) {
					pr_err("%s: prepare enable_clk failed ret:%d\n", __func__, ret);
			   }
			   clk_run = true;
			   pr_err("%s BBCLK2 ON\n", __func__);
			}
	...
	{
		p61_access_state_t current_state = P61_STATE_IDLE;
		NFC_DBG_MSG("%s : P61_GET_PWR_STATUS = %x", __func__, current_state);
		put_user(current_state, (int __user *)arg);
	}
	break;
	default:
		NFC_ERR_MSG("%s :bad ioctl %x\n", __func__, cmd);
		return -EINVAL;
	}
...
static int pn5xx_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (client->dev.of_node) {
		...
		ret = of_get_named_gpio(of_node, "nxp,pn544-irq", 0);
		NFC_DBG_MSG(" of_property_read(irq_gpio) fail: %d, name: %s\n", ret, of_node->name);
		if (ret > 0) {
		...
		client->irq = gpio_to_irq(pn5xx_dev->irq_gpio);
		if (client->irq < 0) {
		pr_info("%s gpio to irq failed", __func__);
		}

		ret = of_get_named_gpio(of_node, "nxp,pn544-fw-dwnld", 0);
		if (ret > 0) {
		...
		ret = of_get_named_gpio(of_node, "nxp,pn544-ven", 0);
		if (ret > 0) {
...
/*
		ret = of_get_named_gpio(of_node, "nxp,pn544-ese-pwr", 0);
		if (ret > 0) {
...
*/
#ifdef ENABLE_NFC_DOUBLE_SIM_SWITCH
		ret = of_get_named_gpio(of_node, "nxp,pn544-swp2-pwr", 0);
		if (ret > 0) {
...
#endif
#ifdef CONFIG_NFC_ENABLE_BB_CLK2
		ret = of_get_named_gpio(of_node, "nxp,pn544-clkreq", 0);
		NFC_DBG_MSG("of_property_read(pn5xx_dev->clkreq_gpio) :%d\n", ret);
		if (ret > 0) {
		...
		source = clk_get(&client->dev, "source");
		if (IS_ERR(source)) {
			NFC_DBG_MSG("Error getting source_clk\n");
			source = NULL;
		} else{
			NFC_DBG_MSG("start prepare soure_clk\n");
		}
		enable = clk_get(&client->dev, "enable");
		if (IS_ERR(enable)) {
			NFC_DBG_MSG("Error getting enable_clk\n");
			enable = NULL;
		} else{
			NFC_DBG_MSG("start prepare enable_clk\n");
		}
#endif
