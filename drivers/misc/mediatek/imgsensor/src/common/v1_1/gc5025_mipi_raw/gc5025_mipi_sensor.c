...
static struct imgsensor_info_struct imgsensor_info = {
	...
	linelength with Capture mode for shutter calculate */
	    ...
	    .mipi_data_lp2hs_settle_dc = 85,/* unit , ns */
	    /*.mipi_pixel_rate = 864000000,*/
		/* less than 13M(include 13M),cap1 max framerate is 24fps,16M max framerate is 20fps,
		20M max framerate is 15fps */
	    .max_framerate = 240,
	},
	.normal_video = {
	   .pclk = 288000000,               /* record different mode's pclk */
...
static void gc5025_gcore_read_otp_info(void)
{
		...
		for (i = 0; i < total_number; i++) {
			...
			if (ddtempbuff[4 * i + 3] & 0x10) {
				...
				default:
					...
					gc5025_otp_info.dd_param_y[cnt] = ((kal_uint16)ddtempbuff[4 * i + 2] << 4)
	...
	for (index = 0; index < 2; index++) {
		...
		case 0x00:
			LOG_INF("GC5025_OTP_WB group %d is Empty !!\n", index + 1);
			gc5025_otp_info.wb_flag = gc5025_otp_info.wb_flag | 0x00;
			break;
		case 0x01:
			LOG_INF("GC5025_OTP_WB group %d is Valid !!\n", index + 1);
			check = 0;
			...
			if ((check % 255 + 1) == wb[WB_WIDTH - 1]) {
				...
				LOG_INF("GC5025_OTP_WB Check sum %d Error !!\n", index + 1);
			}
			break;
		case 0x02:
		case 0x03:
			LOG_INF("GC5025_OTP_WB group %d is Invalid !!\n", index + 1);
		...
		case 0x00:
			LOG_INF("GC5025_OTP_GOLDEN group %d is Empty !!\n", index + 1);
			gc5025_otp_info.golden_flag = gc5025_otp_info.golden_flag | 0x00;
			break;
		case 0x01:
			LOG_INF("GC5025_OTP_GOLDEN group %d is Valid !!\n", index + 1);
			check = 0;
			...
			if ((check % 255 + 1) == golden[GOLDEN_WIDTH - 1]) {
				...
				LOG_INF("GC5025_OTP_GOLDEN Check sum %d Error !!\n", index + 1);
			}
			break;
		case 0x02:
		case 0x03:
			LOG_INF("GC5025_OTP_GOLDEN group %d is Invalid !!\n", index + 1);
