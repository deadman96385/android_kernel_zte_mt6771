...
#include "hi556_mipi_Sensor_125.h"
...
#define PFX "hi556_camera_sensor_125"
...
#define RG_Ratio_Typical 0x149
#define BG_Ratio_Typical 0x11d

kal_uint16 HI556_Sensor_OTP_read(kal_uint16 otp_addr,kal_uint16 otp_addr_one,kal_uint16 otp_addr_two);
void HI556_Sensor_otp_wbdata(void);
void HI556_Sensor_OTP_LSC_data(void);
void HI556_Sensor_OTP_afdata(void);
 void HI556_OTP_cali(void);
#define OTP_SIZE 1869
char hi556_eeprom_data[OTP_SIZE] = { 0 };
kal_uint16 hi556_eeprom_data_awb[8] = { 0 };

kal_uint8 is_otpdata_read = 0;

#endif

static imgsensor_info_struct imgsensor_info = { 
	.sensor_id = HI556_SENSOR_125_ID,
...
static void HI556_OTP_write_cmos_sensor(kal_uint32 addr, kal_uint32 para)
{
...
static struct hi556_otp_struct hi556_otp_info;

void HI556_OTPSetting(void)
{
    LOG_INF("HI556OTPSetting begin:\n");
    HI556_OTP_write_cmos_sensor(0x0A02, 0x01); //fast sleep On
    HI556_OTP_write_cmos_sensor(0x0A00, 0x00); //sleep On
    mDELAY(100);
    HI556_OTP_write_cmos_sensor(0x0F02, 0x00); //pll disable
    HI556_OTP_write_cmos_sensor(0x011A, 0x01); //CP TRI_H
    HI556_OTP_write_cmos_sensor(0x011B, 0x09); //IPGM TRIM_H
    HI556_OTP_write_cmos_sensor(0x0D04, 0x01); //Fsync Output enable
    HI556_OTP_write_cmos_sensor(0x0D00, 0x07); //Fsync Output Drivability
    //HI556_OTP_write_cmos_sensor(0x004C, 0x01); //TG MCU enable
    HI556_OTP_write_cmos_sensor(0x003E, 0x10); //OTP R/W MODE //HI556_OTP_write_cmos_sensor(0x003F, 0x10); //OTP R/W  // 
    HI556_OTP_write_cmos_sensor(0x0A00, 0x01); //sleep off
...
kal_uint16 HI556_Sensor_OTP_read(kal_uint16 otp_addr,kal_uint16 otp_addr_one,kal_uint16 otp_addr_two)
{
    kal_uint16 i, data;
    i = otp_addr; 
    HI556_OTP_write_cmos_sensor(0x10a, (i >> 8) & 0xFF); //start address H        
    HI556_OTP_write_cmos_sensor(0x10b, i & 0xFF); //start address L
    HI556_OTP_write_cmos_sensor(0x102, 0x01); //single read
    //mDELAY(10);
    data = read_cmos_sensor_otp(0x108); //OTP data read
    
	return data;
}


void HI556_Sensor_OTP_info(void)
{
    ...
    info_flag = HI556_Sensor_OTP_read(0x0401,0,0);
    LOG_INF("info_flag = 0x%x---------\n", info_flag);
    switch (info_flag)
    {
        case 0x01:
            ModuleHouseID = HI556_Sensor_OTP_read(0x0402,0,0);
	     sensor_id       = HI556_Sensor_OTP_read(0x0403,0,0);
            Lens_id          = HI556_Sensor_OTP_read(0x0404,0,0);
            vcm_id         = HI556_Sensor_OTP_read(0x0405,0,0);
            devicer_ic_id       = HI556_Sensor_OTP_read(0x0406,0,0);
            Year     = HI556_Sensor_OTP_read(0x0407,0,0);
	     month       = HI556_Sensor_OTP_read(0x0408,0,0);
	     day        = HI556_Sensor_OTP_read(0x0409,0,0);
            break;
        case 0x13:
            ModuleHouseID = HI556_Sensor_OTP_read(0x0412,0,0);
	     sensor_id       = HI556_Sensor_OTP_read(0x0413,0,0);
            Lens_id          = HI556_Sensor_OTP_read(0x0414,0,0);
            vcm_id         = HI556_Sensor_OTP_read(0x0415,0,0);
            devicer_ic_id           = HI556_Sensor_OTP_read(0x0416,0,0);
            Year     = HI556_Sensor_OTP_read(0x0417,0,0);
	     month       = HI556_Sensor_OTP_read(0x0418,0,0);
	     day        = HI556_Sensor_OTP_read(0x0419,0,0);
            break;
        case 0x37:
            ModuleHouseID = HI556_Sensor_OTP_read(0x0422,0,0);
	     sensor_id       = HI556_Sensor_OTP_read(0x0423,0,0);
            Lens_id          = HI556_Sensor_OTP_read(0x0424,0,0);
            vcm_id         = HI556_Sensor_OTP_read(0x0425,0,0);
            devicer_ic_id           = HI556_Sensor_OTP_read(0x0426,0,0);
            Year     = HI556_Sensor_OTP_read(0x0427,0,0);
	     month       = HI556_Sensor_OTP_read(0x0428,0,0);
	     day        = HI556_Sensor_OTP_read(0x0429,0,0);
            break;
        default:	
            LOG_INF("HI556_Sensor: info_flag error value: %d \n ",info_flag);
        break;
    }
    LOG_INF("HI556_ModuleHouseID =0x%x\n", ModuleHouseID);
    LOG_INF("HI556_Year = 0x%x \n", Year);
    LOG_INF("HI556_month = 0x%x \n", month);
    LOG_INF("HI556_day = 0x%x \n", day);	
    LOG_INF("HI556_sensor_id = 0x%x \n", sensor_id);
...
//  kal_uint16 i=0;
	...
	hi556_otp_info.wb_flag = HI556_Sensor_OTP_read(0x1BD2,0,0);

	LOG_INF("awb_info_flag-- = 0x%x--\n", info_flag);


    switch (hi556_otp_info.wb_flag)
    {
        case 0x01:	//Group1 valid
            wb_unit_rg_h   = HI556_Sensor_OTP_read(0x1BD3,0,0);
            wb_unit_rg_l   = HI556_Sensor_OTP_read(0x1BD4,0,0);
            wb_unit_bg_h   = HI556_Sensor_OTP_read(0x1BD5,0,0);
            wb_unit_bg_l   = HI556_Sensor_OTP_read(0x1BD6,0,0);
            wb_unit_gg_h   = HI556_Sensor_OTP_read(0x1BD7,0,0);
            wb_unit_gg_l   = HI556_Sensor_OTP_read(0x1BD8,0,0);
            wb_golden_rg_h = HI556_Sensor_OTP_read(0x1BD9,0,0);
            wb_golden_rg_l = HI556_Sensor_OTP_read(0x1BDA,0,0);
            wb_golden_bg_h = HI556_Sensor_OTP_read(0x1BDB,0,0);
            wb_golden_bg_l = HI556_Sensor_OTP_read(0x1BDC,0,0);
           // wb_golden_gg_h = HI556_Sensor_OTP_read(0x0440,0,0);
          //  wb_golden_gg_l = HI556_Sensor_OTP_read(0x0441,0,0);

         // wbcheck        = HI556_Sensor_OTP_read(0x0453,0,0);

            break;
        case 0x13:	//Group2 valid
            wb_unit_rg_h   = HI556_Sensor_OTP_read(0x1BE6,0,0);
            wb_unit_rg_l   = HI556_Sensor_OTP_read(0x1BE7,0,0);
            wb_unit_bg_h   = HI556_Sensor_OTP_read(0x1BE8,0,0);
            wb_unit_bg_l   = HI556_Sensor_OTP_read(0x1BE9,0,0);
            wb_unit_gg_h   = HI556_Sensor_OTP_read(0x1BEA,0,0);
            wb_unit_gg_l   = HI556_Sensor_OTP_read(0x1BEB,0,0);
            wb_golden_rg_h = HI556_Sensor_OTP_read(0x1BEC,0,0);
            wb_golden_rg_l = HI556_Sensor_OTP_read(0x1BED,0,0);
            wb_golden_bg_h = HI556_Sensor_OTP_read(0x1BEE,0,0);
            wb_golden_bg_l = HI556_Sensor_OTP_read(0x1BEF,0,0);
           // wb_golden_gg_h = HI556_Sensor_OTP_read(0x0440,0,0);
          //  wb_golden_gg_l = HI556_Sensor_OTP_read(0x0441,0,0);
        ...
        case 0x37:	//Group3 valid
            wb_unit_rg_h   = HI556_Sensor_OTP_read(0x1BF9,0,0);
            wb_unit_rg_l   = HI556_Sensor_OTP_read(0x1BFA,0,0);
            wb_unit_bg_h   = HI556_Sensor_OTP_read(0x1BFB,0,0);
            wb_unit_bg_l   = HI556_Sensor_OTP_read(0x1BFC,0,0);
            wb_unit_gg_h   = HI556_Sensor_OTP_read(0x1BFD,0,0);
            wb_unit_gg_l   = HI556_Sensor_OTP_read(0x1BFE,0,0);
            wb_golden_rg_h = HI556_Sensor_OTP_read(0x1BFF,0,0);
            wb_golden_rg_l = HI556_Sensor_OTP_read(0x1C00,0,0);
            wb_golden_bg_h = HI556_Sensor_OTP_read(0x1C0,0,0);
            wb_golden_bg_l = HI556_Sensor_OTP_read(0x1C02,0,0);
           // wb_golden_gg_h = HI556_Sensor_OTP_read(0x0440,0,0);
          //  wb_golden_gg_l = HI556_Sensor_OTP_read(0x0441,0,0);
         ...
         // wbcheck        = HI556_Sensor_OTP_read(0x0453,0,0);
            ...
            break;
        default:
            LOG_INF("HI556_Sensor: wb_flag error value: 0x%x\n ",hi556_otp_info.wb_flag);
        break;
    }

   memset(hi556_eeprom_data_awb, 0, sizeof(hi556_eeprom_data_awb));

hi556_eeprom_data_awb[0]= ((wb_unit_rg_h &0x0ff) << 8) | (wb_unit_rg_l & 0x0ff);
hi556_eeprom_data_awb[1]= ((wb_unit_bg_h &0x0ff) << 8) | (wb_unit_bg_l & 0x0ff);

hi556_eeprom_data_awb[2]= ((wb_unit_gg_h &0x0ff) << 8) | (wb_unit_gg_l & 0x0ff);
hi556_eeprom_data_awb[3]= ((wb_golden_rg_h &0x0ff) << 8) | (wb_golden_rg_l & 0x0ff);
hi556_eeprom_data_awb[4]= ((wb_golden_bg_h &0x0ff) << 8) | (wb_golden_bg_l & 0x0ff);
hi556_eeprom_data_awb[5]= hi556_eeprom_data_awb[2];

    LOG_INF("wb_unit_rg_h = 0x%x, wb_unit_rg_l = 0x%x, wb_unit_bg_h = 0x%x, wb_unit_bg_l = 0x%x, wb_golden_rg_h = 0x%x, wb_golden_rg_l = 0x%x, wb_golden_bg_h = 0x%x, wb_golden_bg_l = 0x%x\n", 
    wb_unit_rg_h, wb_unit_rg_l, wb_unit_bg_h, wb_unit_bg_l, wb_golden_rg_h, wb_golden_rg_l, wb_golden_bg_h, wb_golden_bg_l);
       LOG_INF("wb_unit_gg_h=  0x%x-wb_unit_gg_l=0x%x-----\n",wb_unit_gg_h,wb_unit_gg_l);
}
...
void HI556_Sensor_OTP_afdata(void)
{
    ...
    info_flag = HI556_Sensor_OTP_read(0x0432,0,0);
    LOG_INF("af_info_flag-- = 0x%x--------\n", info_flag);
    switch (info_flag)
    {
        case 0x01:
            mac_h = HI556_Sensor_OTP_read(0x0433,0,0);
	     mac_l       = HI556_Sensor_OTP_read(0x0434,0,0);
            inf_h          = HI556_Sensor_OTP_read(0x0435,0,0);
            inf_l         = HI556_Sensor_OTP_read(0x0436,0,0);
            break;
        case 0x13:
            mac_h = HI556_Sensor_OTP_read(0x043A,0,0);
	     mac_l       = HI556_Sensor_OTP_read(0x043B,0,0);
            inf_h          = HI556_Sensor_OTP_read(0x043C,0,0);
            inf_l         = HI556_Sensor_OTP_read(0x043D,0,0);
            break;
        case 0x37:
            mac_h = HI556_Sensor_OTP_read(0x0441,0,0);
	     mac_l       = HI556_Sensor_OTP_read(0x0442,0,0);
            inf_h          = HI556_Sensor_OTP_read(0x0443,0,0);
            inf_l         = HI556_Sensor_OTP_read(0x0444,0,0);
            break;
        default:	
            LOG_INF("HI556_Sensor: info_flag error value: %d \n ",info_flag);
        break;
    }
    LOG_INF("HI556_Sensor_OTP_afdata  mac_h= 0x%x,mac_l= 0x%x,inf_h= 0x%x,inf_l= 0x%x \n", mac_h,mac_l,inf_h,inf_l);

hi556_eeprom_data_awb[6]= ((mac_h &0x0ff) << 8) | (mac_l & 0x0ff);
hi556_eeprom_data_awb[7]= ((inf_h &0x0ff) << 8) | (inf_l & 0x0ff);
	  
}


void HI556_Sensor_OTP_LSC_data()
{
     kal_uint16 i, addr_start;
     kal_uint16 info_flag     = 0;
   //  kal_uint16 data[1868]={0};

   info_flag = HI556_Sensor_OTP_read(0x0D2F,0,0);

    LOG_INF("otp_info_flag-- = 0x%x----\n", info_flag);
    ...
    HI556_OTP_write_cmos_sensor(0x10a, (addr_start >> 8) & 0xFF); //start address H        
    HI556_OTP_write_cmos_sensor(0x10b, addr_start & 0xFF); //start address L
    HI556_OTP_write_cmos_sensor(0x102, 0x01); //single read

   memset(hi556_eeprom_data, 0, sizeof(hi556_eeprom_data));

    for(i=0;i<1868;i++)
    	{
    	hi556_eeprom_data[i]= read_cmos_sensor_otp(0x108); //OTP data read
	//LOG_INF("HI556_Sensor_otp hi556_eeprom_data[%d]=  0x%x--lhl---1214-LSC-\n",i, hi556_eeprom_data[i]);
    	}
	
}


 void HI556_OTPSetting_end(void)
{
    LOG_INF("HI556_OTPSetting_end begin:---\n");
    HI556_OTP_write_cmos_sensor(0x0A00, 0x00); //stand by on
    mDELAY(10);
    HI556_OTP_write_cmos_sensor(0x003E, 0x00); //display mode 
    HI556_OTP_write_cmos_sensor(0x0A00, 0x01); //sleep off
    LOG_INF("HI556OTPSetting exit :\n");
}
 ...
 void HI556_OTP_cali(void)
{
	LOG_INF("get in HI556_OTP_cali\n");

        HI556_OTPSetting();
        HI556_Sensor_OTP_info();
       HI556_Sensor_otp_wbdata();
	HI556_Sensor_OTP_afdata();
	HI556_Sensor_OTP_LSC_data();
	HI556_OTPSetting_end();
   
   	LOG_INF("get out HI556_OTP_cali\n");
}
...
static void sensor_init(void)
{
	...
	write_cmos_sensor(0x0f30, 0x6925); //pll								   
...
write_cmos_sensor(0x0a22, 0x0000);                                        
write_cmos_sensor(0x0a12, 0x0510); //x output size 1296                   
write_cmos_sensor(0x0a14, 0x03cc); //y output size 972                   
write_cmos_sensor(0x003e, 0x0000);                                        
write_cmos_sensor(0x0804, 0x0200);                                        
...
write_cmos_sensor(0x0a00, 0x0000);
write_cmos_sensor(0x0b0a, 0x8252);
write_cmos_sensor(0x0f30, 0x6925); //pll  //0x6e25                           
write_cmos_sensor(0x0f32, 0x7067); //pll                                  
write_cmos_sensor(0x004a, 0x0100);                                        
write_cmos_sensor(0x004c, 0x0000);                                        
write_cmos_sensor(0x004e, 0x0000); //per-frame control off, on 0x0100      
...
write_cmos_sensor(0x0a00, 0x0100);
	...
	write_cmos_sensor(0x0a00, 0x0000);
write_cmos_sensor(0x0b0a, 0x8252);
write_cmos_sensor(0x0f30, 0x6825); //pll                                  
write_cmos_sensor(0x0f32, 0x7067); //pll                                  
write_cmos_sensor(0x004a, 0x0100);                                        
write_cmos_sensor(0x004c, 0x0000);                                        
write_cmos_sensor(0x004e, 0x0000); //per-frame control off, on 0x0100     
...
UINT32 HI556_MIPI_RAW_125_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
