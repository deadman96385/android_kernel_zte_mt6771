...
//#define FW_IMAGE_NAME "ZTE_A531_Holitech_5P47_1440_720_S3608_63523032.img"
#define FW_IMAGE_NAME "ZTE_A531_Holitech_5P47_1440_720_S3608_63523033.img"
#define DO_STARTUP_FW_UPDATE
#ifdef DO_STARTUP_FW_UPDATE
//#include "ZTE_A531_Holitech_5P47_1440_720_S3608_63523032.h"
#include "ZTE_A531_Holitech_5P47_1440_720_S3608_63523033.h"
#endif
...
static int fwu_parse_image_info(void)
{
	...
	printk("%s: header->major_header_version = (0x%0x)\n",
		...
		unsigned short block_cnt, unsigned char command)
{
	...
	if (fwu->payload_length > (WRITE_SIZE_LIMIT / fwu->block_size))
		max_transfer = WRITE_SIZE_LIMIT / fwu->block_size;
	else
...
static int fwu_enter_flash_prog(void)
{
	...
	if (fwu->in_bl_mode)
		return 0;
#if 0	//comment out 20150703 to avoid write block timeout
...
static int fwu_do_read_customer_serialization_data(void)
{
	...
	printk("%s: Start of customer serialization aquirement process\n", __func__);
	retval = fwu_read_flash_status();
	if (retval < 0)
		goto exit;
	...
	block_count = fwu->blkcount.pm_config;
	if (block_count == 0) {
		printk("%s: Invalid block count\n",__func__);
	       goto exit;
	}
	fwu->config_size = fwu->block_size * block_count;
	printk("%s: Block size = %d\n", __func__, fwu->block_size);
	printk("%s: Permanent config block count = %d\n", __func__, block_count);
	printk("%s: Permanent config size = %d\n", __func__, fwu->config_size);
	printk("%s: permanent config size = %d\n",__func__, fwu->config_size);
	...
	for (ii = 0; ii < 10; ii++)
		printk("%s: Permanent config data[%d] = 0x%x\n", __func__, ii, fwu->read_config_buf[ii]);
	
	module_id = fwu->read_config_buf[2]%16*16+fwu->read_config_buf[3]%16;
	printk("%s:module_id = 0x%x\n", __func__, module_id);
	

exit:

	printk("%s: End of customer serialization acquirement process\n", __func__);
...
static void fwu_startup_fw_update_work(struct work_struct *work)
{
	...
	while (!rmi4_data->fb_ready) {
       ...
       retval = fwu_get_device_config_id();
       module_id = fwu->config_id[1];
	if (retval < 0) {
		dev_err(&fwu->rmi4_data->i2c_client->dev,
				"%s: Failed to read device config ID\n",
				__func__);
	}
	printk("%s : config ID is fwu->config_id[0]=0x%x, fwu->config_id[1]=0x%x,fwu->config_id[2]=0x%x,fwu->config_id[3]=0x%x \n",
		__func__,fwu->config_id[0],fwu->config_id[1],fwu->config_id[2],fwu->config_id[3]);
	if (module_id == 0x00 || module_id == 0xff)
		module_id = fwu_do_read_customer_serialization_data();
	
	printk("%s:module_id = 0x%x\n", __func__, module_id);
	if(module_id == 0x52){    /*config_id[1] == 0x52   ----> Holitech  modify by 10211958*/
		printk("goto fw_updater for holiteck!\n");
		synaptics_fw_updater(FirmwareImageHlt);
   	}
