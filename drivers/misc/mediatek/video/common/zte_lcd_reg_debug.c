...
extern LCM_DRIVER *g_zte_ctrl_pdata;
...
static void zte_lcd_reg_rw_func(struct zte_lcd_reg_debug *reg_debug)
{
	...
	switch ((int)reg_debug->is_read_mode) {
	case REG_READ_MODE:
...
void zte_lcd_reg_debug_func(LCM_DRIVER *lcm_drv, LCM_UTIL_FUNCS *utils)
{
	int ret = -1;
	...
	memcpy(&lcm_util, utils, sizeof(LCM_UTIL_FUNCS));
}
