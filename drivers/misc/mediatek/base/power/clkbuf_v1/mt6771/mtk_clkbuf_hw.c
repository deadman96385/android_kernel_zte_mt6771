...
static void clk_buf_ctrl_bblpm_mask(enum clk_buf_id id, bool onoff)
{
	...
	case CLK_BUF_UFS:
		...
		break;
	default:
		clk_buf_pr_info("%s: id=%d isn't supported\n", __func__, id);
		break;
	}
...
bool clk_buf_ctrl_combine(enum clk_buf_id id, bool onoff)
{
	...
	case CLK_BUF_CONN:
		...
		if (!(pwrap_dcxo_en_flag & DCXO_CONN_ENABLE)) {
			ret = -1;
			clk_buf_pr_info("%s: id=%d skip due to non co-clock for CONN\n", __func__, id);
			pmic_clk_buf_ctrl_wcn(0);
			pmic_clk_buf_swctrl[XO_WCN] = 0;
			break;
		}
	...
	case CLK_BUF_NFC:
		if (onoff)
			pmic_config_interface(PMIC_DCXO_CW00_SET, xo3_mode_set[CO_BUF_M],
					PMIC_XO_EXTBUF3_MODE_MASK,
					PMIC_XO_EXTBUF3_MODE_SHIFT);
		else {
			CLK_BUF3_STATUS_PMIC = CLOCK_BUFFER_SW_CONTROL;
	...
	case CLK_BUF_RF:
		if (onoff)
			pmic_config_interface(PMIC_DCXO_CW00_SET, xo4_mode_set[CO_BUF_M],
					PMIC_XO_EXTBUF4_MODE_MASK,
					PMIC_XO_EXTBUF4_MODE_SHIFT);
		else {
			CLK_BUF4_STATUS_PMIC = CLOCK_BUFFER_HW_CONTROL;
...
void clk_buf_dump_clkbuf_log(void)
{
	u32 pmic_cw00 = 0, pmic_cw02 = 0, pmic_cw11 = 0, pmic_cw14 = 0,
	    pmic_cw16 = 0, pmic_cw23 = 0, top_spi_con1 = 0;
	...
	pmic_read_interface(PMIC_DCXO_CW23, &pmic_cw23,
			    PMIC_REG_MASK, PMIC_REG_SHIFT);
	clk_buf_pr_info("%s CW00/02/11/14/16/23 = 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, top_spi_con1=0x%x\n",
		     __func__, pmic_cw00, pmic_cw02, pmic_cw11, pmic_cw14,
		     pmic_cw16, pmic_cw23, top_spi_con1);
}
				 ...
				 char *buf)
{
	...
	pmic_read_interface_nolock(PMIC_DCXO_CW23, &pmic_cw23,
			    PMIC_REG_MASK, PMIC_REG_SHIFT);
	len += snprintf(buf+len, PAGE_SIZE-len, "DCXO_CW00/02/11/13/14/15/16/23=0x%x %x %x %x %x %x %x %x\n",
