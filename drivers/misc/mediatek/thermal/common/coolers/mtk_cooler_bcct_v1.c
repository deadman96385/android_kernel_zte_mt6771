...
pr_warn("[Thermal/TC/bcct]" fmt, ##args)
			...
			if (chrlmt_registered_users[i]->bat_chr_curr_limit > -1)
				min_bat_char_curr_limit =
					MIN(chrlmt_registered_users[i]->bat_chr_curr_limit
						, min_bat_char_curr_limit);
			if (chrlmt_registered_users[i]->pep30_input_curr_limit > -1)
				min_pep30_input_curr_limit =
					MIN(chrlmt_registered_users[i]->pep30_input_curr_limit
						, min_pep30_input_curr_limit);
		}
...
static void mtk_cl_bcct_set_bcct_limit(void)
{
	...
	if (min_limit != cl_bcct_cur_limit) {
		...
		if (cl_bcct_cur_limit >= 65535) {
			...
			chrlmt_set_limit(&cl_bcct_chrlmt_handle, -1, cl_bcct_cur_limit, -1);
...
static int mtk_cl_abcct_set_cur_temp(struct thermal_cooling_device *cdev, unsigned long temp)
{
	...
	if (cl_abcct_state == 0) {
		...
		abcct_pep30_cur_input_curr_limit = abcct_pep30_max_input_curr_limit;
	...
	if ((limit < abcct_pep30_min_input_curr_limit) && (bat_info_charging_type != 3))
	...
	if ((bat_info_charging_type == 0) && (pep30_reset_to_normal)) {
		/* reset to normal charger. go to do the normal charger thermal limit*/
...
static int mtk_cl_abcct_lcmoff_set_cur_temp(struct thermal_cooling_device *cdev, unsigned long temp)
{
	...
	if (cl_abcct_lcmoff_state == 0) {
		...
		abcct_lcmoff_pep30_cur_input_curr_limit = abcct_lcmoff_pep30_max_input_curr_limit;
	...
	chrlmt_set_limit(&abcct_lcmoff_chrlmt_handle, -1, limit, abcct_lcmoff_pep30_cur_input_curr_limit);
...
static ssize_t _cl_abcct_lcmoff_write(struct file *filp, const char __user *buf, size_t len, loff_t *data)
{
	...
	scan_count =  sscanf(tmp, "%d %ld %ld %ld %ld %d %d %d %d"
		...
		, &_max_cur, &_min_cur, &_pep30_max_input, &_pep30_min_input);
	...
	if (scan_count >= 7) {
		...
		if (scan_count > 7) {
			abcct_lcmoff_pep30_max_input_curr_limit = _pep30_max_input;
			abcct_lcmoff_pep30_min_input_curr_limit = _pep30_min_input;
		}
...
static int _cl_battery_status_read(struct seq_file *m, void *v)
{
	...
	seq_printf(m, "%d,%d,%d,%d,%d,%d,%d,%d,%d,%d\n",
		...
		bat_info_charging_type, bat_info_pep30_curr_limit);
