...
#include "hi846p671f50134_mipi_raw_Sensor.h"
...
#define HI846_DEBUG_OTP 0

#define HI846_OTP_BASE 0x0201
#define HI846_OTP_BASE_LEN 17

#define HI846_OTP_WB 0x0c5f
#define HI846_OTP_WB_LEN 30

#define HI846_OTP_LSC 0x0235
#define HI846_OTP_LSC_LEN 867

#define HI846_RG_Ratio_Typical   0xE0
#define HI846_BG_Ratio_Typical   0x125
bool otp_up_flag = false;
...
struct otp_struct g_current_otp_;
...
static void hi846_otp_read_baseinfo(void)
{
	kal_uint16  base = 0;
#if HI846_DEBUG_OTP
	kal_uint16  val = 0;
...
#endif
	kal_uint16  flag = hi846_otp_read(HI846_OTP_BASE);

	LOG_INF("HI846_OTP baseinfo flag 0x%x\n", flag);
	...
	if (flag == 0x01) {
		...
		LOG_INF(" HI846_OTP base info fail flag %x\n", flag);
	}
#if HI846_DEBUG_OTP
	for (i = 0; i < HI846_OTP_BASE_LEN; i++) {
		val = hi846_otp_read(base + i);
		LOG_INF("HI846_OTP i %d addr 0x%x  val 0x%x\n", i, base+i, val);
		checksum = checksum + val;
		mDELAY(5);
	}
#endif
}
static void hi846_otp_read_wbinfo(void)
{
	kal_uint16  base = 0;
#if HI846_DEBUG_OTP
	kal_uint16  val = 0;
...
#endif
	kal_uint16  flag = hi846_otp_read(HI846_OTP_WB);
...
#endif
	g_current_otp_.rg_ratio = hi846_otp_read(base) << 8 | hi846_otp_read(base + 1);
	g_current_otp_.bg_ratio = hi846_otp_read(base + 2) << 8 | hi846_otp_read(base + 3);
	LOG_INF("HI843B_OTP_AWB rg 0x%x bg 0x%x\n", g_current_otp_.rg_ratio, g_current_otp_.bg_ratio);
}
static void hi846_write_gain(void)
{
	kal_uint16   R_gain, G_gain, B_gain;

	LOG_INF("HI843B_OTP_AWB rg 0x%x bg 0x%x\n", g_current_otp_.rg_ratio, g_current_otp_.bg_ratio);
	if (g_current_otp_.rg_ratio == 0x0 || g_current_otp_.bg_ratio == 0x0) {
		LOG_INF("HI846_OTP_AWB read fail current rg bg is 0\n");
	}
	R_gain = 0x200;
	B_gain = 0x200;
	G_gain = 0x200;

	R_gain = (0x200 * HI846_RG_Ratio_Typical) / g_current_otp_.rg_ratio;
	B_gain = (0x200 * HI846_BG_Ratio_Typical) / g_current_otp_.bg_ratio;
	...
	LOG_INF("HI846_OTP_AWB %s,after calcuate: R_gain = 0x%x, G_gain = 0x%x, B_gain = 0x%x\n",
		__func__, R_gain, G_gain, B_gain);
	LOG_INF("HI846_OTP_AWB %s exit\n", __func__);
	if (G_gain >= 0x200) {
...
static void hi846_otp_read_lscinfo(void)
{
	kal_uint16  flag = hi846_otp_read(HI846_OTP_LSC);
	kal_uint16  base = 0;
	kal_uint16  val = 0;
#if HI846_DEBUG_OTP
	kal_uint16  i = 0;
	kal_uint16  checksumwb = 0;
#endif
	LOG_INF("HI846_OTP_LSC lscinfo flag 0x%x\n", flag);
	...
	if (flag == 0x01) {
		...
		LOG_INF(" HI846_OTP_LSC lscinfo fail flag 0x%x\n", flag);
	}
	val = hi846_otp_read(0x021C);
	if (!val)
		LOG_INF(" HI846_OTP_LSC lsc update success 0x%x\n", val);
	else
		LOG_INF(" HI846_OTP_LSC lsc update fail 0x%x\n", val);
#if HI846_DEBUG_OTP
	for (i = 0; i < HI846_OTP_LSC_LEN; i++) {
		val = hi846_otp_read(base + i);
		mDELAY(5);
		LOG_INF("HI846_OTP_LSC lsc addr 0x%x 0x%x\n", base + i, val);
		checksumwb = checksumwb + val;
	}
#endif

}
static void hi846_otp_update(void)
{
	LOG_INF("E\n");

	if (!otp_up_flag) {
		hi846_otp_setting();
		hi846_otp_read_baseinfo();
		hi846_otp_read_wbinfo();
		hi846_otp_read_lscinfo();
		hi846_otp_disable();
		otp_up_flag = true;
	}
	hi846_write_gain();
...
UINT32 HI846P671F50134_MIPI_RAW_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
