...
static s8 fts_sensor_enter_doze(struct i2c_client *client)
{
    ...
    ret = fts_i2c_write_reg(i2c_client, FT_GESTRUE_MODE_SWITCH_REG, gestrue_on);
    if (ret < 0)
    {
    ...
    for (i = 0; i < 10; i++)
    {
        fts_i2c_read_reg(i2c_client, FT_GESTRUE_MODE_SWITCH_REG, &gestrue_data);
        ...
        msleep(20);
        fts_i2c_write_reg(i2c_client, FT_GESTRUE_MODE_SWITCH_REG, gestrue_on);
...
void fts_sensor_enable(struct i2c_client *client)
{
    ...
    data = 0x00;
    fts_i2c_write_reg(client, FT_GESTRUE_MODE_SWITCH_REG, data);
}
...
void fts_sensor_suspend(struct i2c_client *i2c_client)
{
    ...
    if (scp_init_flag == 0)
    {
        ...
        FTS_DEBUG("[TOUCH]SEND CUST command :%d ", IPI_COMMAND_AS_CUST_PARAMETER);
    ...
    if (tpd_scp_doze_en)
    {
        FTS_DEBUG("[TOUCH]SEND ENABLE GES command :%d ", IPI_COMMAND_AS_ENABLE_GESTURE);
