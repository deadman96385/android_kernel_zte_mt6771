...
static int charger_policy_recovery_charging(struct charger_policy_info *policy_info)
{
	...
	if (rc < 0) {
		pr_policy("Failed to set charging enabled property rc=%d\n", rc);
	}
	pr_policy("\n");
...
static int charger_policy_ctrl_cas(struct charger_policy_info *policy_info, bool status)
{
#ifdef CONFIG_CHARGE_ARBITRATE_SERVICE
	union power_supply_propval val = {0, };
	int rc = 0;
	...
	val.intval = status;
	rc = power_supply_set_property(policy_info->cas_psy,
...
static int charger_policy_update_usb_status(struct charger_policy_info *policy_info)
{
	...
	if (!policy_info->usb_present && usb_present) {
		...
		if (policy_info->battery_charging_policy_ops.charging_policy_status & DEMO_CHARGING_POLICY) {
			...
			alarm_start_relative(&policy_info->charging_demo_alarm, ms_to_ktime(0));
		}
		...
		if ((policy_info->battery_charging_policy_ops.charging_policy_status & DEMO_CHARGING_POLICY) ||
			...
			charger_policy_ctrl_cas(policy_info, false);
...
static int charger_policy_notifier_handler(struct charger_policy_info *policy_info)
{
	...
	if ((policy_info->battery_charging_policy_ops.charging_policy_status & DEMO_CHARGING_POLICY)) {
		...
		charger_policy_ctrl_cas(policy_info, true);
							...
							ktime_t now)
{
	...
	queue_delayed_work(policy_info->timeout_workqueue, &policy_info->timeout_work, msecs_to_jiffies(50));
...
#ifdef CONFIG_CHARGE_ARBITRATE_SERVICE
	policy_info->cas_psy = power_supply_get_by_name(policy_info->config_prop.cas_phy_name);
	if (!policy_info->cas_psy) {
		pr_policy("Get cas psy failed\n");
		goto battery_psy_failed;
	}
#endif
...
#ifdef CONFIG_CHARGE_ARBITRATE_SERVICE
	power_supply_put(policy_info->cas_psy);
#endif
#endif
	devm_kfree(policy_info->dev, policy_info);
	policy_info = NULL;
