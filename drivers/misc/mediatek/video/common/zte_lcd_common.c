...
extern void zte_lcd_reg_debug_func(LCM_DRIVER *lcm_drv, LCM_UTIL_FUNCS *utils);
#endif

LCM_DRIVER *g_zte_ctrl_pdata = NULL;
...
static int zte_lcd_proc_info_display(LCM_DRIVER *lcm_drv)
{
...
static void zte_lcd_common_init(LCM_DRIVER *lcm_drv)
{
	int ret = 0;
...
void zte_lcd_common_func(LCM_DRIVER *lcm_drv, LCM_UTIL_FUNCS *utils)
{
	if (g_zte_ctrl_pdata) {
		return;
	}
