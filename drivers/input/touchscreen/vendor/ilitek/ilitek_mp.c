...
static struct mp_test_items tItems[MP_TEST_ITEM] = {
	...
	{.name = "mutual_signal", .desp = "untouch signal data(bg-raw-4096) - mutual", .result = "FAIL",
		.catalog = MUTUAL_TEST},
	...
	{.name = "self_signal", .desp = "untouch signal data(bg-raw-4096) - self", .result = "FAIL",
		.catalog = SELF_TEST},
	...
	{.name = "noise_peak_to_peak_ic", .desp = "noise peak to peak(ic only)", .result = "FAIL",
		.catalog = PEAK_TO_PEAK_TEST},
	{.name = "noise_peak_to_peak_panel", .desp = "noise peak to peak(with panel)", .result = "FAIL",
		.catalog = PEAK_TO_PEAK_TEST},
	{.name = "noise_peak_to_peak_ic_lcm_off", .desp = "noise peak to peak(ic only) (lcm off)", .result = "FAIL",
		.catalog = PEAK_TO_PEAK_TEST},
	{.name = "noise_peak_to_peak_panel_lcm_off", .desp = "noise peak to peak(with panel) (lcm off)",
		.result = "FAIL", .catalog = PEAK_TO_PEAK_TEST},
	{.name = "mutual_no_bk_lcm_off", .desp = "raw data(no bk) (lcm off)", .result = "FAIL", .catalog = MUTUAL_TEST},
	{.name = "mutual_has_bk_lcm_off", .desp = "raw data(have bk) (lcm off)", .result = "FAIL",
		.catalog = MUTUAL_TEST},
	...
	{.name = "doze_p2p_td_lcm_off", .desp = "peak to peak_td (lcm off)", .result = "FAIL",
		.catalog = PEAK_TO_PEAK_TEST},
...
static int parser_get_ini_key_value(char *section, char *key, char *value)
{
	...
	for (i = 0; i < g_ini_items; i++) {
		...
		if (strcmp(key, ilitek_ini_file_data[i].pKeyName) == 0) {
			ipio_memcpy(value, ilitek_ini_file_data[i].pKeyValue, ilitek_ini_file_data[i].iKeyValueLen,
...
void parser_ini_nodetype(s32 *type_ptr, char *desp, int frame_len)
{
	...
	for (i = 0; i < g_ini_items; i++) {
		if ((strnstr(ilitek_ini_file_data[i].pSectionName, desp, PARSER_MAX_KEY_NAME_LEN) <= 0)
		...
		for (j = 0, index1 = 0; j <= ilitek_ini_file_data[i].iKeyValueLen; j++) {
			if (ilitek_ini_file_data[i].pKeyValue[j] == ';' || j == ilitek_ini_file_data[i].iKeyValueLen) {
				if (record != '.') {
					...
					ipio_memcpy(str, &ilitek_ini_file_data[i].pKeyValue[index1],
						(j - index1), sizeof(str));
					temp = katoi(str);
	...
	for (i = 0; i < g_ini_items; i++) {
		...
		for (j = 0, index1 = 0; j <= ilitek_ini_file_data[i].iKeyValueLen; j++) {
			if (ilitek_ini_file_data[i].pKeyValue[j] == ','
				|| ilitek_ini_file_data[i].pKeyValue[j] == ';'
				|| ilitek_ini_file_data[i].pKeyValue[j] == '.'
				|| j == ilitek_ini_file_data[i].iKeyValueLen) {
...
static int parser_get_int_data(char *section, char *keyname, char *rv, size_t rv_size)
{
	int len = 0;
	char value[512] = { 0 };
	...
	if (parser_get_ini_key_value(section, keyname, value) < 0) {
		snprintf(rv, rv_size, "%s", value);
		return 0;
	}

	len = snprintf(rv, rv_size, "%s", value);
	return len;
}
...
static char *parser_ini_str_trim_r(char *buf)
{
	...
	strlcpy(buf, tmp, PARSER_MAX_CFG_BUF + 1);
	return buf;
}
	...
	while (true) {
		...
		if (n > 2 && ((ini_buf[0] == M_CFG_SSL && ini_buf[n - 1] != M_CFG_SSR))) {
			...
			if (ini_buf[0] == M_CFG_SSL) {
				...
				ini_buf[n - 1] = 0x00;
				strlcpy((char *)tmpSectionName, ini_buf + 1, PARSER_MAX_CFG_BUF + 1);
				banchmark_flag = 0;
				nodetype_flag = 0;
		...
		strlcpy(ilitek_ini_file_data[g_ini_items].pSectionName,
		...
		if (isEqualSign == 0) {
			...
			if (strnstr(ilitek_ini_file_data[g_ini_items].pSectionName,
					BENCHMARK_KEY_NAME, PARSER_MAX_KEY_NAME_LEN) > 0) {
				banchmark_flag = 1;
				isEqualSign = -1;
			} else if (strnstr(ilitek_ini_file_data[g_ini_items].pSectionName,
					NODE_TYPE_KEY_NAME, PARSER_MAX_KEY_NAME_LEN) > 0) {
				nodetype_flag = 1;
				isEqualSign = -1;
			} else {
				continue;
		...
		if (banchmark_flag) {
			...
			strlcpy(ilitek_ini_file_data[g_ini_items].pKeyName,
...
#define DATE_TIME_STR_SIZE 128
static char *get_date_time_str(void)
{
	struct timespec now_time;
	struct rtc_time rtc_now_time;
	static char time_data_buf[DATE_TIME_STR_SIZE] = { 0 };

	getnstimeofday(&now_time);
	rtc_time_to_tm(now_time.tv_sec, &rtc_now_time);
	snprintf(time_data_buf, DATE_TIME_STR_SIZE, "%04d%02d%02d-%02d%02d%02d",
...
static void mp_print_csv_header(char *csv, int *csv_len, int *csv_line)
{
	...
	tmp_line++;
	tmp_len += snprintf(csv + tmp_len, CSV_FILE_SIZE - tmp_len, "Confidentiality Notice:\n");
	tmp_line++;
	tmp_len += snprintf(csv + tmp_len, CSV_FILE_SIZE - tmp_len,
...
#define MP_CSV_STR_SIZE 128
static void mp_print_csv_cdc_cmd(char *csv, int *csv_len, int index)
{
	int i, slen = 0, tmp_len = *csv_len;
	char str[MP_CSV_STR_SIZE] = {0};
	const char * const open_sp_cmd[] = {"open dac", "open raw1", "open raw2", "open raw3"};
	const char * const open_c_cmd[] = {"open cap1 dac", "open cap1 raw"};
	char *name = tItems[index].desp;

	if (strncmp(name, "open test(integration)_sp", strlen(name)) == 0) {
		for (i = 0; i < ARRAY_SIZE(open_sp_cmd); i++) {
			slen = parser_get_int_data("pv5_4 command", (char *)open_sp_cmd[i], str, MP_CSV_STR_SIZE);
			if (slen < 0)
		...
		for (i = 0; i < ARRAY_SIZE(open_c_cmd); i++) {
			slen = parser_get_int_data("pv5_4 command", (char *)open_c_cmd[i], str, MP_CSV_STR_SIZE);
			if (slen < 0)
				ipio_err("Failed to get CDC command %s from ini\n", open_sp_cmd[i]);
			else
				tmp_len += snprintf(csv + tmp_len, CSV_FILE_SIZE - tmp_len,
		...
		slen = parser_get_int_data("pv5_4 command", name, str, MP_CSV_STR_SIZE);
		if (slen < 0)
			ipio_err("Failed to get CDC command %s from ini\n", name);
		else
			tmp_len += snprintf(csv + tmp_len, CSV_FILE_SIZE - tmp_len,
...
void allnode_open_cdc_result(int index, int *buf, int *dac, int *raw)
{
	...
	if (strncmp(name, "open_integration_sp", strlen(name)) == 0) {
		...
		for (i = 0; i < core_mp.frame_len; i++)
			buf[i] = idev->chip->open_c_formula(dac[i], raw[i], open_c_spec.tvch - open_c_spec.tvcl,
				open_c_spec.gain);
	}
}
...
static int codeToOhm(s32 Code, u16 *v_tdf, u16 *h_tdf)
{
	...
	if (Code == 0) {
		...
		temp = ((douTVCH - douTVCL) * douVariation * (douTDF1 - douTDF2)
			* (1 << 12) / (9 * Code * douCint)) * 100;
...
static int mp_cdc_get_pv5_4_command(u8 *cmd, int len, int index)
{
	...
	slen = parser_get_int_data("pv5_4 command", key, str, MP_CSV_STR_SIZE);
	if (slen < 0)
		return -EPERM;
...
static int allnode_open_cdc_data(int mode, int *buf)
{
	...
	char str[MP_CSV_STR_SIZE] = {0};
	char tmp[128] = {0};
	const char * const key[] = {"open dac", "open raw1", "open raw2", "open raw3",
	...
	ret = parser_get_int_data("pv5_4 command", (char *)key[mode], str, MP_CSV_STR_SIZE);
	if (ret < 0) {
...
int check_int_level(bool high)
{
	...
	while (timer) {
		...
		if (high) {
			if (level) {
				ipio_info("check int high success\n");
				ret = 0;
				break;
			}
		} else {
			if (!level) {
				ipio_info("check int low success\n");
				ret = 0;
				break;
			}
		}
...
out:
	...
	ipio_info("Change to default IRQ trigger type\n");
	ilitek_plat_irq_unregister();
...
static int open_test_sp(int index)
{
	...
	ret = parser_get_int_data(tItems[index].desp, "charge_aa", str, OPEN_TEST_STR_SIZE);
	if (ret || ret == 0)
		Charge_AA = katoi(str);

	ret = parser_get_int_data(tItems[index].desp, "charge_border", str, OPEN_TEST_STR_SIZE);
	if (ret || ret == 0)
		Charge_Border = katoi(str);

	ret = parser_get_int_data(tItems[index].desp, "charge_notch", str, OPEN_TEST_STR_SIZE);
	if (ret || ret == 0)
		Charge_Notch = katoi(str);

	ret = parser_get_int_data(tItems[index].desp, "full open", str, OPEN_TEST_STR_SIZE);
	if (ret || ret == 0)
	...
	for (i = 0; i < tItems[index].frame_count; i++) {
		...
		ilitek_dump_data(&tItems[index].buf[(i * core_mp.frame_len)], 10, core_mp.frame_len,
			core_mp.xch_len, "after full_open_rate_compare");

		addr = 0;
		for (y = 0; y < core_mp.ych_len; y++) {
			for (x = 0; x < core_mp.xch_len; x++) {
				tItems[index].buf[(i * core_mp.frame_len) + addr] = compare_charge(open[i].charg_rate,
					x, y, tItems[index].node_type, Charge_AA, Charge_Border, Charge_Notch);
				addr++;
			}
		}

		ilitek_dump_data(&tItems[index].buf[(i * core_mp.frame_len)], 10, core_mp.frame_len,
			core_mp.xch_len, "after compare charge rate");
...
static int open_test_cap(int index)
{
	...
	ret = parser_get_int_data(tItems[index].desp, "gain", str, OPEN_TEST_STR_SIZE);
	if (ret || ret == 0)
		open_c_spec.gain = katoi(str);

	ret = parser_get_int_data(tItems[index].desp, "tvch", str, OPEN_TEST_STR_SIZE);
	if (ret || ret == 0)
		open_c_spec.tvch = katoi(str);

	ret = parser_get_int_data(tItems[index].desp, "tvcl", str, OPEN_TEST_STR_SIZE);
	if (ret || ret == 0)
...
#define MP_GET_TIMIMG_STR_SIZE 256
static int mp_get_timing_info(void)
{
	int slen = 0;
	char str[MP_GET_TIMIMG_STR_SIZE] = {0};
	u8 info[64] = {0};
	char *key = "timing_info_raw";

	core_mp.isLongV = 0;

	slen = parser_get_int_data("pv5_4 command", key, str, MP_GET_TIMIMG_STR_SIZE);
	if (slen < 0)
		return -EPERM;
...
#define CSV_NAME_SIZE 128
#define FRAME_NAME_SIZE 128
static int mp_show_result(bool lcm_on)
{
	...
	for (i = 0; i < ARRAY_SIZE(tItems); i++) {
		...
		if (strcmp(tItems[i].name, "open_integration_sp") == 0) {
			mp_compare_cdc_show_result(i, frame1_cbk700, csv, &csv_len, TYPE_NO_JUGE,
				max_threshold, min_threshold, "frame1 cbk700");
			mp_compare_cdc_show_result(i, frame1_cbk250, csv, &csv_len, TYPE_NO_JUGE,
				max_threshold, min_threshold, "frame1 cbk250");
			mp_compare_cdc_show_result(i, frame1_cbk200, csv, &csv_len, TYPE_NO_JUGE,
				max_threshold, min_threshold, "frame1 cbk200");
		}

		if (strcmp(tItems[i].name, "open test_c") == 0) {
			mp_compare_cdc_show_result(i, cap_dac, csv, &csv_len, TYPE_NO_JUGE,
				max_threshold, min_threshold, "CAP_DAC");
			mp_compare_cdc_show_result(i, cap_raw, csv, &csv_len, TYPE_NO_JUGE,
				max_threshold, min_threshold, "CAP_RAW");
		}
		...
		if (tItems[i].catalog == KEY_TEST) {
			...
			mp_compare_cdc_show_result(i, core_mp.tx_max_buf, csv, &csv_len, TYPE_JUGE,
				max_threshold, min_threshold, "TX Max Hold");
			mp_compare_cdc_show_result(i, core_mp.tx_min_buf, csv, &csv_len, TYPE_JUGE,
				max_threshold, min_threshold, "TX Min Hold");
			...
			for (j = 0; j < get_frame_cont; j++) {
				char frame_name[FRAME_NAME_SIZE] = {0};

				snprintf(frame_name, FRAME_NAME_SIZE, "Frame %d", (j+1));
				mp_compare_cdc_show_result(i, &tItems[i].buf[(j*core_mp.frame_len)], csv,
					&csv_len, TYPE_NO_JUGE, max_threshold, min_threshold, frame_name);
			}
		}
	}
	...
	if (pass_item_count == 0) {
		...
		ret = MP_FAIL;
		if (lcm_on)
			snprintf(csv_name, CSV_NAME_SIZE,
				"%s/%s_%s.csv", CSV_LCM_ON_PATH, get_date_time_str(), ret_fail_name);
		else
			snprintf(csv_name, CSV_NAME_SIZE,
				"%s/%s_%s.csv", CSV_LCM_OFF_PATH, get_date_time_str(), ret_fail_name);
	} else {
		core_mp.final_result = MP_PASS;
		ret = MP_PASS;
		if (lcm_on)
			snprintf(csv_name, CSV_NAME_SIZE,
				"%s/%s_%s.csv", CSV_LCM_ON_PATH, get_date_time_str(), ret_pass_name);
		else
			snprintf(csv_name, CSV_NAME_SIZE,
				"%s/%s_%s.csv", CSV_LCM_OFF_PATH, get_date_time_str(), ret_pass_name);
	}
...
#define ILITEK_MP_RESULT_SIZE 16
static void ilitek_tddi_mp_init_item(void)
{
	...
	for (i = 0; i < MP_TEST_ITEM; i++) {
		...
		tItems[i].result = kmalloc(ILITEK_MP_RESULT_SIZE, GFP_KERNEL);
		snprintf(tItems[i].result, ILITEK_MP_RESULT_SIZE, "%s", "FAIL");
	}
...
static void mp_test_run(char *item)
{
	...
	for (i = 0; i < MP_TEST_ITEM; i++) {
		if (strncmp(item, tItems[i].desp, strlen(item)) == 0) {
			...
			parser_get_int_data(item, "enable", str, OPEN_TEST_STR_SIZE);
			tItems[i].run = katoi(str);
			parser_get_int_data(item, "spec option", str, OPEN_TEST_STR_SIZE);
			tItems[i].spec_option = katoi(str);
			parser_get_int_data(item, "type option", str, OPEN_TEST_STR_SIZE);
			tItems[i].type_option = katoi(str);
			parser_get_int_data(item, "frame count", str, OPEN_TEST_STR_SIZE);
			tItems[i].frame_count = katoi(str);
			parser_get_int_data(item, "trimmed mean", str, OPEN_TEST_STR_SIZE);
			tItems[i].trimmed_mean = katoi(str);
			parser_get_int_data(item, "lowest percentage", str, OPEN_TEST_STR_SIZE);
			tItems[i].lowest_percentage = katoi(str);
			parser_get_int_data(item, "highest percentage", str, OPEN_TEST_STR_SIZE);
			tItems[i].highest_percentage = katoi(str);

			/* Get pin test delay time */
			if (tItems[i].catalog == PIN_TEST) {
				parser_get_int_data(item, "test int pin", str, OPEN_TEST_STR_SIZE);
				tItems[i].test_int_pin = katoi(str);
				parser_get_int_data(item, "int pulse test", str, OPEN_TEST_STR_SIZE);
				tItems[i].int_pulse_test = katoi(str);
				parser_get_int_data(item, "delay time", str, OPEN_TEST_STR_SIZE);
				tItems[i].delay_time = katoi(str);
			}

			/* Get TDF value from ini */
			if (tItems[i].catalog == SHORT_TEST) {
				parser_get_int_data(item, "v_tdf_1", str, OPEN_TEST_STR_SIZE);
				tItems[i].v_tdf_1 = parser_get_tdf_value(str, tItems[i].catalog);
				parser_get_int_data(item, "v_tdf_2", str, OPEN_TEST_STR_SIZE);
				tItems[i].v_tdf_2 = parser_get_tdf_value(str, tItems[i].catalog);
				parser_get_int_data(item, "h_tdf_1", str, OPEN_TEST_STR_SIZE);
				tItems[i].h_tdf_1 = parser_get_tdf_value(str, tItems[i].catalog);
				parser_get_int_data(item, "h_tdf_2", str, OPEN_TEST_STR_SIZE);
				tItems[i].h_tdf_2 = parser_get_tdf_value(str, tItems[i].catalog);
			} else {
				parser_get_int_data(item, "v_tdf", str, OPEN_TEST_STR_SIZE);
				tItems[i].v_tdf_1 = parser_get_tdf_value(str, tItems[i].catalog);
				parser_get_int_data(item, "h_tdf", str, OPEN_TEST_STR_SIZE);
				tItems[i].h_tdf_1 = parser_get_tdf_value(str, tItems[i].catalog);
			}

			/* Get threshold from ini structure in parser */
			if (strcmp(item, "tx/rx delta") == 0) {
				parser_get_int_data(item, "tx max", str, OPEN_TEST_STR_SIZE);
				core_mp.TxDeltaMax = katoi(str);
				parser_get_int_data(item, "tx min", str, OPEN_TEST_STR_SIZE);
				core_mp.TxDeltaMin = katoi(str);
				parser_get_int_data(item, "rx max", str, OPEN_TEST_STR_SIZE);
				core_mp.RxDeltaMax = katoi(str);
				parser_get_int_data(item, "rx min", str, OPEN_TEST_STR_SIZE);
				...
				parser_get_int_data(item, "max", str, OPEN_TEST_STR_SIZE);
				tItems[i].max = katoi(str);
				parser_get_int_data(item, "min", str, OPEN_TEST_STR_SIZE);
...
int ilitek_tddi_mp_test_main(char *apk, bool lcm_on)
{
	...
	ilitek_ini_file_data = vmalloc(sizeof(struct ini_file_data) * PARSER_MAX_KEY_NUM);
